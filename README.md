...
| Shielded pool | SHAKE-256 commitments and nullifiers, SP1 raw FRI-STARK, no elliptic-curve ZK; per-output note ciphertexts are **ML-KEM-1024** (FIPS 203, Category 5) + AES-256-GCM — Category 5 because a ciphertext on-chain faces harvest-now-decrypt-later forever. The KEM lives only in the wallet layer (`bloch-crypto`'s opt-in `note-crypto` feature): the node validates ciphertext *structure* and never decapsulates, so the transport row's "no ML-KEM in the node" claim still holds. | Proofs are large and proving is expensive, and each shielded output carries a 1,668-byte note ciphertext. The mainnet pool is provably empty — nothing has ever been shielded. |
