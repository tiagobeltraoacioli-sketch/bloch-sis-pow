...
# v2 is in the genesis validator set but never runs while the fixture is built.
# Its keystore is what the node under test boots with, so that node is a real
# committee member with real duties for every slot it is behind on — which is
# the mainnet shape: a validator that fell behind, not a passive observer.
"$BIN" keygen --dir "$FIX/n2" --index 2 >"$FIX/keygen2.log" 2>&1 || { echo "keygen2 FAILED"; cat "$FIX/keygen2.log"; exit 1; }
"$BIN" genesis --keys "$FIX/n0,$FIX/n1,$FIX/n2" --out "$FIX/genesis.json" --slot-ms "$SLOT_MS" --start-in 5 \
