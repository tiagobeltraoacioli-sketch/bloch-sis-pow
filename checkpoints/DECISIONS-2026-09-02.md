...
> ## CORRECTIONS, 2026-09-02 (later the same day)
>
> This memo was written earlier on 2026-09-02 and ported onto the release
> lineage (tag `g4-node-20260901`) afterwards. Three of its statements were
> re-measured against the code and do not survive. They are corrected here
> rather than edited away, because a decision memo that quietly changes is
> worse than one that shows its corrections.
>
> **1. §2's "every valid Phase A quorum contains that party — the client
> enforces this; there is no ceremony-side workaround" is FALSE as written.**
> Every client enforces the `threshold` and `min_external` stated in the
> *arrangement file it was given*, not the values §6 states.
> `ws::matches_policy` is the function that compares the two, and — confirmed
> by grep over the whole tree, not assumed — **no acceptance path calls it**;
> its only callers were tests. Two consequences:
> - an arrangement file with `min_external = 0` makes a Foundation + Postern
>   quorum verify on every client, silently;
> - an arrangement seating ONE key in TWO slots is a 1-of-3, because the
>   quorum counts distinct *indices* and nothing anywhere compared two slots'
>   key bytes. Seat the duplicate once internal and once external and the
>   external minimum falls too.
>
> The auditor is therefore a mandatory signer **of the arrangement that is
> actually published**, and the arrangement reaches a node over the same
> unauthenticated channel as the envelope. This does not change the memo's
> conclusion that contacting the auditor is the critical path; it changes what
> must be published alongside the envelope. `ws_boot::decode_signer_set_file`
> now refuses a duplicate-key arrangement outright (covering every node, with
> no change to the frozen committee crate), `ws-signer-set` refuses to write
> one, and a node loading an arrangement that matches neither §6 phase now
> says so loudly at boot. None of that helps someone handed a different file —
> which is why the arrangement's SHA3-256 fingerprint must be published and
> compared exactly like the ws digest. See `docs/CHECKPOINT-RUNBOOK.md` §2.
>
> **2. §3's `adopted_epoch` failure mode is real but understated.** It is not
> only "anywhere near `u64::MAX`". `hard_stop()` is `adopted_epoch + 41,040`,
> so *any* large value pushes the dead-man's switch past every epoch the chain
> will reach: 10^12 epochs is about thirty million years and overflows
> nothing. Reading the three lines back before publishing is a good habit but
> not a mitigation, because a doctored file reaches a node without passing
> through anyone's eyes. The bound is now enforced in code, at the only place
> it can be given there is no flag on the release lineage: the checkpoint's
> epoch must lie in `[adopted_epoch, hard_stop()]`, checked in
> `ws_boot::arrangement_window` at boot and at assembly, plus a refusal in
> `decode_signer_set_file` for a value that saturates the arithmetic outright.
>
> **3. Decision 4 ("sign 1536 now rather than waiting 18 h for 1792") was
> priced against an 18-hour wait.** As of ~18:40 UTC the chain is at epoch
> 1775 and epoch 1792 begins at 19:23 UTC — under an hour — finalizing about
> half an hour after that, still roughly 2.5 days inside the deadline. The
> recommendation is now to sign **1792**, with 1536 as the fallback if the
> holders are available now and not later. Publishing 1536 does not foreclose
> 1792: a higher epoch is `Acceptance::Store`. See `docs/CHECKPOINT-RUNBOOK.md`
> §10 for the arithmetic and the re-measurement recipe.
>
> **A hazard this memo does not mention at all**, added to the runbook as §7:
> `issued_at` is inside `ws_digest` though verification never reads it, and
> `ws::accept` keys on the epoch — so re-minting an already-published epoch
> yields `Acceptance::Conflict` and **refuses the boot of every node that
> stored the first artifact**, with two perfectly valid quorums attesting the
> same history. There is no such thing as a provisional checkpoint. Correct a
> bad publication at the next publication epoch, never at the same one.
