...
fn build(a: &Args) -> Result<(Index, BuildTiming), String> {
    ...
    ix.replay = Some(manifest.genesis_state());
    let t_seed = t2.elapsed();

    let t3 = Instant::now();
    let (applied, _) = ix.sync(&mut reader).map_err(|e| format!("applying log: {e}"))?;
    ix.verify_undo_roundtrip(&mut reader, 13).map_err(|e| format!("undo qualification: {e}"))?;
...
fn serve(a: &Args) -> Result<(), String> {
    ...
    let args = a.clone();
    let poll = std::time::Duration::from_millis(a.poll_ms.clamp(250, 10_000));
    let writer = Arc::clone(&shared);
    std::thread::spawn(move || {
        let mut reader = match LogReader::open(&args.log) {
            Ok(r) => r,
        ...
        loop {
            ...
            if let Err(e) = result {
                ...
                match build(&args) {
                    Ok((mut rebuilt, _)) => {
                        rebuilt.stats.rebuilds += 1;
                        rebuilt.checked_at = Instant::now();
                        *writer.write().unwrap() = rebuilt;
