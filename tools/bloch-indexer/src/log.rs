...
impl LogFingerprint {
    ...
    fn from_metadata(m: std::fs::Metadata) -> io::Result<LogFingerprint> {
        #[cfg(unix)]
        {
...
impl LogReader {
    ...
    pub fn open(path: &Path) -> io::Result<LogReader> {
        ...
        let fingerprint = LogFingerprint::from_metadata(file.metadata()?)?;
        let (frames, end) = scan_frames(&mut file, fingerprint.len)?;
        if end == ScanEnd::ShortFrame {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "invalid short frame in archival log"));
        }
