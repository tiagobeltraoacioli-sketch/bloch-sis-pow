...
|------------|-------------------------------------------------------------------|--------------|
| `deposit`  | funded validator registration (`DepositV2`, wire tag `0x07`)      | full flow; format INERT until `FUNDED_STAKING_ACTIVATION_EPOCH` is armed |
...
## Refusal reference — what the tool refuses before it touches a key

Every row below is decided at **build time**, from the chain's committed
state, and none of them can be overridden by a flag. The point is that a
transaction the chain would reject is never signed, never broadcast, and
never costs an operator a round trip: the refusal names the constant so the
next question ("is this me or is this the chain?") is already answered.

| Refusal | Fires when | Names | Test |
|---|---|---|---|
| format not active | the format's flag day has not bound at the chain's epoch | `FUNDED_STAKING_ACTIVATION_EPOCH` (deposit, delegate), `SIGNED_EXIT_ACTIVATION_EPOCH` (exit), `WITHDRAWAL_ACTIVATION_EPOCH` (withdraw) — and whether it is INERT | `deposit_refuses_when_the_format_is_not_active_naming_the_epoch`, `withdraw_gate_names_its_epoch_and_rehearsal_never_broadcasts`, `exit_gate_names_its_epoch_and_broadcast_refusal_names_the_gap`, `delegate_refusal_names_the_gate_and_the_missing_format`, `every_staking_format_gate_matches_consensus_and_ships_inert` |
| bond below minimum | `amount_sat < MIN_DEPOSIT_SAT` (25,000 BLCH) | `MIN_DEPOSIT_SAT` | `deposit_refuses_below_minimum` |
| bond above the cap | `amount_sat > deposit_cap_sat(total_active)` — 1% of committed active stake, floored at the minimum | the cap in BLCH and that it is **derived by the consensus crate** from the chain's `total_active_stake_sat` | `deposit_refuses_above_the_chain_derived_cap` |
| sub-dust change | change would be `> 0` but `< 546 sat` | the **exact two alternatives**: bond less (change lands exactly on the dust floor) or bond more (consumes the coins exactly, no change) | `deposit_refuses_sub_dust_change_with_exact_alternatives` |
| before `withdrawable_epoch` | the chain's epoch has not reached the record's **committed** `withdrawable_epoch` | both epochs, the gap, and that any slashing included meanwhile EXTENDS the committed field | `withdraw_refuses_before_withdrawable_epoch_naming_both_epochs` |

Two honest notes on that table:

- **The dust floor is this tool's policy, not a Genesis-4 consensus rule.**
  Grep the consensus crate: there is no transfer-level dust constant and no
  minimum-output check on the deposit path. 546 sat is inherited from the
  Genesis-3 incident where one sub-dust transaction failed every block that
  included it. The refusal is therefore prudence, and it says "refusing",
  never "consensus refuses" — the distinction matters if you are ever
  tempted to raise it.
- **The cap has a known divergence, flagged upstream rather than hidden.**
  `transition::deposit_cap_sat` is the naive 1% against *uncapped* active
  stake, while `delegation::Registry::cap_sat` resolves the cap against
  *capped* stake by fixed-point iteration and is materially stricter under
  high concentration. The tool imports the former because it is what the
  transition enforces today. Unifying them is consensus work and belongs on
  the same flag day as `FUNDED_STAKING_ACTIVATION_EPOCH`.

## This tool is the STAKING withdrawal — not the exchange payout

`withdraw` here means "return an exited validator's bond to the withdrawal
credentials fixed at its deposit" — `PosTransaction::Withdraw`, wire tag
`0x08`, a free unauthenticated crank with no inputs, no change and no fee.

It is **not** the exchange customer withdrawal. That is a different product
(`crates/bloch-withdraw`, branch `wt/exchange-payout-client`): a payout state
machine that builds `Transfer`/`TransferV2` from a hot wallet with
at-most-once semantics. The two share a word and nothing else — no wire tag,
no consensus surface, no code. Before writing a third "withdrawal" tool,
establish which of these two you actually mean.

Why the staking crank needs none of that payout machinery: consensus itself
makes it at-most-once. The `Withdraw` arm resets `withdrawable_epoch` to
`u64::MAX` on payout, and that reset **is** the committed withdraw-once
marker — so a replayed crank is refused by the chain, not by the client.
...
- **"funded deposits are not active … FUNDED_STAKING_ACTIVATION_EPOCH"** —
