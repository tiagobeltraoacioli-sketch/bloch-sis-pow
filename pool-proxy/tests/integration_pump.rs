...
/// has something to talk to), each handled on its own task.
async fn spawn_mock_node(en1: &'static str) -> (String, tokio::task::JoinHandle<()>) {
    ...
    let handle = tokio::spawn(async move {
        loop {
            match listener.accept().await {
                Ok((sock, _)) => {
                    tokio::spawn(handle_node_conn(sock, en1.to_string()));
                }
                Err(_) => break,
            }
        }
...
async fn handle_node_conn(sock: TcpStream, en1: String) {
        ...
        match method {
            "mining.subscribe" => {
                ...
                let _ = wr
                    ...
                    .await;
                let _ = wr
                    .write_all(b"{\"id\":null,\"method\":\"mining.notify\",\"params\":[\"job1\",\"ph\",\"c1\",\"c2\",[],\"20000000\",\"1a2b3c4d\",\"5f5e100\",true]}\n")
                    .await;
...
#[tokio::test]
async fn end_to_end_records_accepted_share_then_survives_hostile_input() {
    let (node_addr, _node) = spawn_mock_node("aaaaaaaa").await;
    ...
    // Real subscribe → authorize → submit sequence through the live pump.
    ...
    wr.write_all(b"{\"id\":2,\"method\":\"mining.authorize\",\"params\":[\"bloch1qtest\",\"x\"]}\n")
        .await
        .unwrap();
    wr.write_all(b"{\"id\":3,\"method\":\"mining.submit\",\"params\":[\"w\",\"job1\",\"00000000\",\"5f5e100\",\"00c0ffee\"]}\n")
        .await
        .unwrap();
    wr.flush().await.unwrap();

    // Wait until the pool-wide ledger reflects the accepted share (proves the
    // submit-result was correlated and folded into PPLNS accounting).
    ...
    assert!(recorded, "accepted share should land in the pool-wide PPLNS ledger");
    assert!(
    ...
    assert_eq!(
        ...
        1,
        "only the real submit should be counted"
    );
...
async fn forced_extranonce_collision_triggers_redial_or_unresolved() {
    ...
    let (node_addr, _node) = spawn_mock_node("c0111de0").await;
    let cfg = test_cfg(node_addr);
    let metrics = Arc::new(Metrics::new());
    let registry = Arc::new(ExtranonceRegistry::new());
    let ledger = Arc::new(PplnsLedger::new(&cfg));
