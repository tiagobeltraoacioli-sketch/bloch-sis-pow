...
try{
  ...
  const cacheKeys=await page.evaluate(async()=>{const cache=await caches.open('bloch-pay-shell-v2');return(await cache.keys()).map(r=>new URL(r.url).pathname);});assert(cacheKeys.includes('/app/integrations'));assert(cacheKeys.includes('/app/'));
