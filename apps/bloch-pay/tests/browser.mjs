...
try{
  ...
  await page.locator('#save-route').click();await page.waitForFunction(()=>document.querySelector('#saved-route-list').textContent.includes('cross-border-001'));assert.equal(await page.locator('#studio-metrics strong').nth(1).innerText(),'1');
  assert((await page.locator('#route-planning-gaps').innerText()).includes('2 GAPS'));
  for(const [id,name,role,rail,jurisdiction]of [['origin-psav','<img src=x onerror=alert(1)>','psav','pix','Brazil'],['destination-bank','Example destination bank','bank','sepa','SEPA region']]){
    await page.locator('#new-partner').click();const partner=page.locator('#partner-form');for(const[key,value]of Object.entries({id,name,jurisdiction,owner:'Integration team'}))await partner.locator(`[name=${key}]`).fill(value);await partner.locator('[name=role]').selectOption(role);await partner.locator(`[name=rails][value=${rail}]`).check();
    for(const key of ['entity','access','security','funding','operations'])await partner.locator(`[name=${key}_state]`).selectOption('documented');
    await partner.getByRole('button',{name:'Save participant',exact:true}).click();assert((await partner.locator('.form-error').innerText()).includes('evidence reference'));
    for(const key of ['entity','access','security','funding','operations'])await partner.locator(`[name=${key}_reference]`).fill('review-'+key);
    await partner.getByRole('button',{name:'Save participant',exact:true}).click();await page.locator('#partner-dialog').waitFor({state:'hidden'});
  }
  assert.equal(await page.locator('#partner-list img').count(),0);assert.equal(await page.locator('#corridor-map svg').count(),1);assert.equal(await page.locator('#studio-metrics strong').first().innerText(),'2');assert((await page.locator('#route-planning-gaps').innerText()).includes('0 GAPS'));
  const invoiceBeforeStudioImport=await page.evaluate(()=>localStorage.getItem('bloch-pay-workspace-v1'));
  const studioDownloadWait=page.waitForEvent('download');await page.locator('#export-studio').click();await(await studioDownloadWait).saveAs(artifacts+'/studio-backup.json');const studioBackup=JSON.parse(await readFile(artifacts+'/studio-backup.json','utf8'));assert.equal(studioBackup.partners.length,2);assert.equal(studioBackup.routes.length,1);assert.equal(studioBackup.routes[0].events.length,5);
  await page.reload({waitUntil:'networkidle'});assert.equal(await page.locator('#studio-metrics strong').first().innerText(),'2');await page.locator('[data-open-route]').first().click();assert.equal(await page.locator('#sample-status').innerText(),'returned');assert((await page.locator('#saved-route-state').innerText()).includes('Saved locally'));
  await page.locator('[data-archive-route]').first().click();await page.locator('#studio-confirm-action').click();assert.equal(await page.locator('#studio-metrics strong').nth(1).innerText(),'0');assert.equal(await page.locator('#studio-metrics strong').nth(3).innerText(),'1');
  await page.locator('#route-status-filter').selectOption('archived');await page.locator('[data-archive-route]').first().click();await page.locator('#studio-confirm-action').click();assert.equal(await page.locator('#studio-metrics strong').nth(1).innerText(),'1');await page.locator('#route-status-filter').selectOption('active');
  const forged=structuredClone(studioBackup);forged.routes[0].draft.execution_enabled=true;await page.locator('#studio-backup-file').setInputFiles({name:'forged.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify(forged))});await page.waitForFunction(()=>document.querySelector('#route-toast').textContent.includes('Import refused'));assert.equal(await page.locator('#studio-metrics strong').nth(1).innerText(),'1');
  await page.locator('#studio-backup-file').setInputFiles(artifacts+'/studio-backup.json');await page.locator('#studio-confirm').waitFor({state:'visible'});assert((await page.locator('#studio-confirm-copy').innerText()).includes('2 participants and 1 routes'));await page.locator('#studio-confirm-action').click();await page.locator('#studio-confirm').waitFor({state:'hidden'});assert.equal(await page.evaluate(()=>localStorage.getItem('bloch-pay-workspace-v1')),invoiceBeforeStudioImport);await page.locator('[data-open-route]').first().click();
  for(const width of [1440,390,320]){await page.setViewportSize({width,height:1000});await page.locator('#saved-routes').scrollIntoViewIfNeeded();const size=await page.evaluate(()=>({width:innerWidth,scroll:document.documentElement.scrollWidth}));assert(size.scroll<=size.width,JSON.stringify(size));await page.screenshot({path:`${artifacts}/route-library-${width}.png`});}
  ...
  const cacheKeys=await page.evaluate(async()=>{const cache=await caches.open('bloch-pay-shell-v4');return(await cache.keys()).map(r=>new URL(r.url).pathname);});assert(cacheKeys.includes('/app/integrations'));assert(cacheKeys.includes('/app/'));
  await context.setOffline(true);await page.reload({waitUntil:'networkidle'});assert.equal(await page.locator('#route-form').count(),1);assert.equal(await page.locator('#studio-metrics strong').first().innerText(),'2');await page.locator('[data-open-route]').first().click();assert.equal(await page.locator('#sample-status').innerText(),'returned');await page.goto(base+'/app/',{waitUntil:'networkidle'});assert.equal(await page.locator('.metric strong').first().innerText(),'75.00000001');assert((await page.locator('#connection').innerText()).includes('Offline'));await context.setOffline(false);
