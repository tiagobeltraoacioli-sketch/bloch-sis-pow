...
function invalidate() { computeId++; reviewGeneration++; current=null; review=null; selectedKey=null; $('review-editor').hidden=true; $('reviewer').value=''; $('review-note').value=''; $('import-review').value=''; for(const id of ['result-rows','review-history','review-key','report-digest'])$(id).replaceChildren(); $('review-status').textContent=''; $('review-io-status').textContent='Download the evidence and review journal before clearing, loading files or rerunning. Resume retained reports with Verify below.'; $('results').hidden=true; $('run').disabled=!sources.every(Boolean)||pending.some(Boolean); }
...
function renderTable() {
  if(!current)return;
  const filter=$('status-filter').value,latestByKey=latestReviews(review);
  const rows=current.report.result.items.filter(item=>filter==='all'||(filter==='review'?item.status!=='matched':item.status===filter));
...
setupVerification(({evidence,sources:originals,review:journal})=>{
