...
function invalidate() { computeId++; current=null; $('results').hidden=true; $('run').disabled=!sources.every(Boolean)||pending.some(Boolean); }
...
async function run() {
  ...
  try {
    ...
    current=result; page=0; $('status-filter').value='all'; render();
...
function render() {
  ...
  $('report-digest').textContent=current.digest; renderChart(result); renderTable(); $('results').hidden=false;
}
function renderTable() {
  ...
  for(const item of rows.slice(page*pageSize,(page+1)*pageSize)) {
    ...
    records.append(title,list);detail.append(records);tr.append(key,outcome,refs,detail);tbody.append(tr);
  }
...
$('clear').addEventListener('click',()=>{clearInputs();mode='local_files';updateMode();status('Files and generated results cleared from this page. Downloaded files remain on your device.');});
