...
function invalidate() { computeId++; reviewGeneration++; current=null; review=null; selectedKey=null; queueIndex=null; resetQueueFilters(); $('review-editor').hidden=true; $('reviewer').value=''; $('review-note').value=''; $('import-review').value=''; for(const id of ['result-rows','review-history','review-key','report-digest','field-chart','review-chart','queue-summary'])$(id).replaceChildren(); $('review-status').textContent=''; $('review-io-status').textContent='Download the evidence and review journal before clearing, loading files or rerunning. Resume retained reports with Verify below.'; $('results').hidden=true; $('run').disabled=!sources.every(Boolean)||pending.some(Boolean); }
