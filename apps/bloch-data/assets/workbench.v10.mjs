...
import {setupPreparation} from './preparation-workbench.v1.mjs';
import {setupPreparationVerification} from './preparation-verifier-workbench.v1.mjs';
import {setupAuditBundles} from './audit-bundle-workbench.v2.mjs';
import {setupEncryptedBundles} from './encrypted-bundle-workbench.v1.mjs';
import {setupCaseOverview} from './overview-workbench.v1.mjs';
...
function openPreparedSources(prepared){
  clearInputs();config=validateConfig(prepared.configuration);setConfigurationControls();renderModule();
  sources=prepared.sources;mode=prepared.mode;updateMode();
  $('name-a').textContent=sources[0].name;$('name-b').textContent=sources[1].name;$('run').disabled=false;
  status('Prepared CSVs loaded. Run reconciliation to create evidence bound to these prepared files. Retain the separate preparation receipt and original extracts for lineage.');
  $('workbench').scrollIntoView({block:'start'});
}
setupPreparation(()=>config,openPreparedSources,download);
setupPreparationVerification(()=>config,openPreparedSources,download);
const auditBundles=setupAuditBundles(()=>config,openVerifiedReport,download);
setupEncryptedBundles(()=>auditBundles.getPreparedBundle(),openVerifiedReport,download);
setupCaseOverview(openVerifiedReport,download);
