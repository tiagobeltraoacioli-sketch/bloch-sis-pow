...
export function Dashboard() {
  const { data, error, loading } = useAsync(load, [], 15000);
  ...
  const stalled = ageSecs > STALL_THRESHOLD_SECS;
    ...
    <div className="container">
      ...
      {stalled && (
        <div className="banner-stale">
          ...
          <div>
            <strong>Chain appears stalled.</strong> The newest block is {fmtDuration(ageSecs)} old
            (target ~{fmtInt(net ? 30 : 30)}s). The DAG has {fmtInt(dag?.tip_count ?? 0)} open tips at
            height {fmtInt(tipHeight)} — a consensus fix is deploying separately. Data below is live and honest.
          </div>
        </div>
      )}
      ...
      <div className="grid stat-grid">
        <Stat
          label="Tip height"
          icon={<span className={"dot " + (stalled ? "stale" : "live")} />}
        ...
        />
        <Stat label="Hashrate" value={fmtHashrate(chain?.hashrate_hs ?? 0)} sub={`difficulty ${fmtNum(diff, 0)}`} />
        <Stat
      ...
      {d.window && d.window.blocks.length > 0 && (
        <>
          <div className="section-title">DAG near the tip</div>
      ...
      <div className="rail" style={{ marginTop: 28 }}>
        ...
        Confirmation depth carries no strong security guarantee today. This is an independent reference
