...
class CursorProbeError extends Error {}
...
async function request(endpoint, method, params, id, timeoutMs, fetcher) {
  ...
  if (!response.ok) throw new CursorProbeError(Number.isInteger(response.status) ? `HTTP ${response.status}` : 'HTTP error');
  const reader = response.body?.getReader();
  if (!reader) throw new CursorProbeError('Empty HTTP response body');
  const chunks = [];
  let size = 0;
  try {
    while (true) {
      ...
      if (size > MAX_BYTES) throw new CursorProbeError('Response exceeds 128 KiB limit');
      chunks.push(value);
    }
  } finally {
  ...
  catch { throw new CursorProbeError('Invalid JSON response'); }
  if (payload?.jsonrpc !== '2.0' || payload.id !== id ||
    Object.hasOwn(payload, 'result') === Object.hasOwn(payload, 'error')) {
    throw new CursorProbeError('Invalid JSON-RPC envelope or mismatched request id');
  }
  if (Object.hasOwn(payload, 'error')) {
    if (!payload.error || typeof payload.error !== 'object' || Array.isArray(payload.error) ||
      !Number.isSafeInteger(payload.error.code) || typeof payload.error.message !== 'string') {
      throw new CursorProbeError('Invalid JSON-RPC error');
    }
...
function buildMarker(result) {
  if (!result || typeof result !== 'object' || Array.isArray(result)) throw new CursorProbeError('Expected object build information');
  ...
  if (!Array.isArray(result.features) || result.features.length > 64 ||
    ...
    throw new CursorProbeError('Invalid build feature list');
  }
...
function pageShape(result, scriptHash) {
  if (!result || typeof result !== 'object' || Array.isArray(result)) throw new CursorProbeError('Expected object result');
  if (!validHex32(result.script_hash) || result.script_hash.toLowerCase() !== scriptHash) throw new CursorProbeError('script_hash mismatch');
  if (!validCount(result.total) || !validCount(result.returned) || result.returned > 1 || result.returned > result.total) throw new CursorProbeError('Invalid total or returned count');
  if (typeof result.truncated !== 'boolean' || !Array.isArray(result.utxos) || result.utxos.length !== result.returned) throw new CursorProbeError('Invalid truncated or utxos');
  for (const utxo of result.utxos) {
    if (!utxo || typeof utxo !== 'object' || Array.isArray(utxo) || !validHex32(utxo.txid) ||
      ...
      throw new CursorProbeError('Invalid UTXO entry');
    }
  }
  ...
  if (!extension.every(key => Object.hasOwn(result, key))) throw new CursorProbeError('Partial cursor extension');
  if (!validHex32(result.at_head) || !validCount(result.at_slot)) throw new CursorProbeError('Invalid cursor head or slot');
  if (result.truncated !== (result.next_cursor !== null)) throw new CursorProbeError('truncated and next_cursor disagree');
  if (result.next_cursor !== null) {
    const token = result.next_cursor;
    if (typeof token !== 'string' || !/^01[0-9a-fA-F]{200}$/.test(token)) throw new CursorProbeError('Invalid version-1 cursor token');
    if (token.slice(2, 66).toLowerCase() !== result.at_head.toLowerCase() ||
      token.slice(66, 130).toLowerCase() !== scriptHash) throw new CursorProbeError('Cursor head or script hash mismatch');
    if (result.returned !== 1 || result.total <= result.returned ||
      token.slice(130, 194).toLowerCase() !== result.utxos[0].txid.toLowerCase() ||
      parseInt(token.slice(194, 202), 16) !== result.utxos[0].vout) throw new CursorProbeError('Cursor does not bind to last UTXO');
  }
...
async function checkCursorCapability({ rpc, scriptHash, optIn = false, timeoutMs = 12000, fetcher = fetch }) {
  if (!optIn) throw new CursorProbeError('Explicit --probe-cursor opt-in required before any RPC request');
  const endpoint = endpointUrl(rpc);
  if (!validHex32(scriptHash)) throw new CursorProbeError('Supply a 64-hex --script-hash');
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 500 || timeoutMs > 30000) throw new CursorProbeError('Timeout must be 500–30000 ms');
  ...
  try {
    ...
    report.build_error = error instanceof CursorProbeError ? error.message : 'RPC request failed or response could not be read';
  }
  try {
    report.request_count++;
    ...
    if (page.next_cursor === null) {
      if (page.total !== page.returned) throw new CursorProbeError('First cursor page ended before the reported total');
    ...
    if (next.kind !== 'cursor_shape' || next.at_head !== page.at_head || next.at_slot !== page.at_slot ||
      ...
      throw new CursorProbeError('Second page conflicts with first page');
    }
    ...
    report.error = error instanceof CursorProbeError ? error.message : 'RPC request failed or response could not be read';
    return report;
  }
}
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else throw new CursorProbeError(`Unknown argument: ${arg}`);
  }
  return args;
}
