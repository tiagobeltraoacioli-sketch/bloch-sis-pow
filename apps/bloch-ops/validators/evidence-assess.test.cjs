...
const domain = 'a'.repeat(64), genesis = 'b'.repeat(64), root = 'c'.repeat(64), digest = 'd'.repeat(64);
...
const checkpoint = () => ({
  ...
  command: { name: 'ws-verify', exitCode: 0, stopReason: null }, freshness: 'FRESH', wsDigest: digest, expectedDigest: digest, inputFingerprints: { genesis: { sha256: genesis } },
