...
Supply both 64-character digests from independently authenticated release material. The tool checks each bundle's `SHA256SUMS`, schema and manual gate; checks that the preflight's network domain matches the independently supplied domain; and checks that the manifest file fingerprint recorded by `checkpoint-verify.cjs` matches the independently supplied SHA-256. It also compares the checkpoint verifier's printed epoch, root, digest, freshness and acceptance with its structured report and the preflight's reported chain epoch and finalized checkpoint. A passing preflight summary must include the expected passing head, finality and admission checks, the unresolved manual trust check, and one successful RPC diagnostic per required method and endpoint. The structured head, lag, finality and admission fields must satisfy those passing checks. A passing checkpoint report must have the four named passing checks, an unambiguous successful command result, and a checkpoint epoch no later than the verifier's node clock epoch. These consistency checks reject contradictory or incomplete bundles; they do not authenticate their creator.
