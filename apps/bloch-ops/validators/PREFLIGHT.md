...
node preflight.cjs \
  ...
  --expect-source-digest "$BLOCH_RELEASE_SOURCE_DIGEST"
```

Set `BLOCH_NETWORK_DOMAIN` and `BLOCH_RELEASE_SOURCE_DIGEST` from independently authenticated Genesis-4 release material before running the example. Both flags are optional, but each supplied value must be exactly 64 hexadecimal characters. A public RPC response is not a trusted source for either value. The digest comparison uses the node's self-reported `getbuildinfo.source_digest`: a match checks the claimed source identity only. It does not authenticate the running binary or its artifact digest. Replace the example local URL with your node's actual RPC URL. Remote URLs must use HTTPS. Never expose a private node RPC unauthenticated on the internet.
