...
| **Supply inflation beyond MAX_SUPPLY** | MITIGATED (per-block) | Each block's coinbase is capped at `miner_reward(h) + fees` and `treasury_reward(h)`. Halving schedule caps cumulative supply at 21M. GAP: no global invariant enforces ∑ coinbase == expected total at the current height — a divergence would only be caught when supply distribution is sampled. Sprint O covers this. |
