...
> **Provenance note, 2026-09-01.** This file reached branch `rpc/build-identity`
> by `git checkout pmo/wire-namespace-registry -- docs/WIRE-NAMESPACE-REGISTRY.md`
> — the document alone, not that branch's code, which touches `engine.rs`,
> `slashing.rs` and `transition.rs` and is out of scope here. It was copied
> because `rpc.rs` points every agent adding a method at this file and the file
> did not exist on the RPC lineage at all. §5 below is edited; if the PMO branch
> lands later, git will conflict at exactly the edited section, which is the
> correct outcome and not an accident.

Served today (mainline, **16** methods — the "13" this section used to say
predates `getcapabilities`, `gettxout` and `getbuildinfo`):

`getbalance`, `getblockbyid`, `getblockbyslot`, `getblockcount`,
`getbuildinfo`, `getcapabilities`, `getchaininfo`, `getmempoolinfo`,
`getnewaddress`, `gettransaction`, `gettxout`, `getutxos`, `getvalidator`,
`getvalidatorcount`, `listunspent`, `sendrawtransaction`.

**Deployment caveat, and it is the reason this list is dangerous to read alone.**
"Served today" describes the source, not the fleet. Measured 2026-09-01 against
both public archivals and all nine RPC upstreams: `getcapabilities` answers
`-32601` on every one of them, and it is not in the published tag
`g4-node-20260901` (`7a83ca89`) at all. `RPC_SURFACE` itself exists only on the
`validator-ops` lineage — `main` (`ffd074a5`), the fleet binary (`46133196`) and
the release tag do not contain it. A registry that says "served" when it means
"in some branch" is the same defect the registry exists to prevent, one level
up. Every row here should be read as *claimed in source*, and the wire checked
separately.

### `getbuildinfo` — **CLAIMED 2026-09-01, branch `rpc/build-identity`**

| | |
| --- | --- |
| Method | `getbuildinfo` |
| Status | built, unmerged; base `16d36638` (`validator-ops`) merged with `gates/selfcheck-json` |
| Dispatch | `rpc.rs`, arm immediately after `getcapabilities` |
| Handler | `rpc::build_info_json` |
| Engine | `engine.rs`, `RpcRequest::BuildInfo` |
| Frozen by | `the_rpc_method_namespace_is_frozen` (`rpc/tests.rs`) both directions, plus `apps/explorer/edge/tests/surface-frozen.test.mjs` across the language boundary |
| Edge | `EDGE_SURFACE`, cache class `node_local`, corroboration `none` |
| Surface version | 4.1.0 → **4.2.0** |

Answers "which binary is this, and which consensus lineage is it on": the build
commit from `BLOCH_BUILD_VERSION`, and the consensus gate table with its
SHA3-256 digest. **Do not rename it** and do not add a second version method —
`getversion` was probed on the live endpoint and is deliberately absent, and two
names for build identity is how a client ends up with two answers.

**`RPC_SURFACE_VERSION` is a namespace too.** It collides the same silent way a
method name does, and it did: `rpc/build-identity` and uncommitted work in the
shared checkout at `~/dev/BlochPOS` both moved it `4.1.0` → `4.2.0`, for
unrelated reasons, and the two bumps touch different lines so git merges them
without a word. **Whichever lands second renumbers to `4.3.0` in the landing
commit.**
