...
- **Nothing computes such a root anywhere in this repository**, and
  checkpoint-sync state download — wired as of 2026-08-31 — deliberately does
  not consume it: `state_sync` verifies a downloaded snapshot by recomputing
  the state SMT and comparing against the checkpoint's `state_root` alone
  (`state_sync::import`, which writes `validator_set_root: [0; 32]`). Since
  the registry is committed leaf-by-leaf inside `state_root`, that one root
  already verifies the downloaded registry. So there is still no producer and
  no consumer of `validator_set_root`.
...
**The forward hazard, written down so it is not discovered later:** the
field survives for a future registry-only fast path — one that verifies a
downloaded registry against `validator_set_root` *without* rebuilding the
state tree. The day that path is built, it must **refuse** an all-zero
`validator_set_root` rather than treat it as "matches". A naive
