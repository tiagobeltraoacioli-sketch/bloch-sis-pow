...
Status:     DRAFT — milestone plan for review; **still no code exists for
            either track.** Re-verified 2026-08-14: `crates/bloch-pos-node`
            references `euvm` zero times and neither PoS crate depends on it.
            Genesis-4 launched on 2026-08-13 **without an EVM and without
            Ustav at L1**; the header's `EvmCommitment` is carried, never
            recomputed — a reserved slot, not an execution layer.
Created:    2026-08-11
Owner:      PMO
