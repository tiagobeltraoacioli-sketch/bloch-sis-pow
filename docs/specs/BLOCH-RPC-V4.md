...
- `finalized` — the containing block is an ancestor of (or equal to) the
  ...
  **not irreversible.** This bullet used to read "This is the only state that
  ...
  rewind, measured descending 6 -> 4 -> 2 -> 0. There is no machine-readable
  form of this on the released node: it serves no `getcapabilities` method, so
  a client cannot branch on a flag and must be told in prose.
- `unknown` — not found.
