...
Local source tests enumerate 2,001 outputs in three pages (1,000/1,000/1), check ordering, uniqueness, response counts and cursor boundaries, and reject a cursor from another committed head. Routing tests reject malformed, wrong-version, wrong-script and wrong-type tokens before they reach the backend. These tests do not establish support on the deployed mainnet binary.
