...
this repository. Read from the live chain on 2026-09-01 at epoch 1726 from two
agreeing archivals: 64 validators, 64 active, every record `slashed: false`
and `exit_epoch: null`. Equivocation on this fleet is *detected* and never
prosecuted. (A figure of "48 double-signing validators" has circulated
internally; it is **not substantiated** in this repository, no RPC exposes
equivocation history, and the only equivocation recorded here is two indices
in `deploy/FLAG-DAY-EPOCH-800.md`. Nothing above rests on the number.) Closing
