...
- The carryover set is 18,146,400,000 BLCH across **15 addresses** and 448,337
  UTXOs. The largest address holds 17,046,829,380 BLCH — **93.94% of the
...
- Founder total across carryover plus the new 10% grant (2-year cliff,
  8-year linear vest): **27.04% of eventual total supply**.
...
*shares*. Independent parties hold 227,709,400 / 18,146,400,000 = **6.03%** of
...
**What remains open, and why this finding is reopened.** Both fixes above are
...
`bloch-pos-committee/src/params.rs` — at `u64::MAX` until 2026-09-09, and
since then ARMED at epoch 2700 (`docs/VALIDATOR-LIFECYCLE-FLAG-DAY.md`) —
makes any block carrying evidence consensus-invalid
(`TxReject::EvidenceNotActive`) at every epoch below 2700. (An earlier
revision of this paragraph reported the constant as defined only
off-lineage, on `d21c3370`; the 2026-09-05 change introduced it on this
lineage, unarmed, together with the decodable format.) That decoder
...
Equivocation on this fleet is *detected* and never prosecuted. **The auditor
should ask for the forensics, because the registry understates this
materially:** replay of `blocks.log` puts 48 validators with provable
double-signing, derived seven independent ways — two separately written
forensic pipelines (one producing the pair table with slot, both roots and the
blocks carrying each half), the committed state reproduced, and six
independently kept logs deriving the same set in slot order. The two indices
in `deploy/FLAG-DAY-EPOCH-800.md` (16 and 35) fall inside that set. All 48
read `slashed: false`, which is the finding: detection works, prosecution has
never been possible, and no RPC exposes equivocation evidence, so the gap is
invisible to anyone querying the chain rather than replaying it. Closing
