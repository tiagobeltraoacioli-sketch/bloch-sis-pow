...
| 11 | **The blocking supply-chain CI gates are red** on the current tree (rustls RUSTSEC-2026-0285, published 2026-09-14) — Medium by impact, listed here because it blocks every fix above from merging. | — | yes | LD-01 |
...
**Totals.** 197 findings + 3 from the dynamic run: 15 High, 35 Medium, 96 Low, 52 Info (lead severities; 1 refuted). 120 are labelled NEW, 65 KNOWN or KNOWN-refined against the eleven prior internal audits in `docs/audit/`. Test suites of the live crates: 1,349 passed, 0 failed (Annex A12).
...
Plus **LD-01** (Annex A12 §A12.6): blocking supply-chain gates red on the tree — `rustls 0.23.38`, RUSTSEC-2026-0285, reachable via `libp2p-websocket`; `cargo update -p rustls` resolves it.
...
14. **Get the blocking supply-chain gates green again**: `cargo update -p rustls` (RUSTSEC-2026-0285, published 2026-09-14, fails both `cargo-audit (blocking)` and `cargo-deny (blocking)` today), prune the 289 stale `Cargo.lock` entries with `cargo update --workspace`, and delete the ten `deny.toml`/`audit.toml` ignores that match nothing in the resolved graph. LD-01, LD-02, INF-05.
