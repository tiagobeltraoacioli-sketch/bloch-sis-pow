...
The sections below preserve the first-wave scope and validation (`203b410`). See [second-wave implementation and limits](WAVE-2.md), [third-wave corrections](WAVE-3.md), [fourth-wave corrections](WAVE-4.md), and [protocol blockers](PROTOCOL-BLOCKERS.md) for subsequent work. The finding ledger tracks all waves; a local implementation is not a production security closure.
