...
Obtain the expected fingerprint independently from a trusted publication or operator channel. Reading a hash alongside a file downloaded from the same untrusted source does not establish trust. No authoritative production fingerprint has been asserted or installed by this remediation. The optional pin preserves historical checkpoint signatures and unpinned operation; unpinned supplied arrangements print a warning. A pin without both an incoming checkpoint and an arrangement is refused: it cannot retroactively authenticate an isolated cached checkpoint.
