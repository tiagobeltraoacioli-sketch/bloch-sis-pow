...
An absent local block is not evidence of agreement: the node prints that the checkpoint state root has not been locally validated. This does not implement state download or shorten replay. The full checkpoint now remains in the engine and is rechecked whenever live canonical application or branch adoption makes its named block available, even before finality reaches the checkpoint epoch. A hard trust-anchor contradiction exits 78 for operator action; a node that booted with own finality reports the contradiction without reorganizing or exiting. Boot-time mismatches remain a refusal.

The existing published `validator_set_root = 0` encoding means that no independent validator registry commitment is available. It remains accepted. Existing nonzero signed roots remain accepted with an explicit warning that this field cannot be independently derived. New `ws-checkpoint` creation refuses a manually supplied nonzero `--validator-set-root`; signing an arbitrary operator value must not look like validating it. This is a deliberate CLI restriction, not a wire-format migration. Full SR-05 closure still requires a specified, independently derived validator-set commitment and authenticated validator-root validation.
...
A successfully decoded prefix is **not** a verified chain. This command does not execute consensus transitions, validate state roots or signatures, or provide missing per-frame checksums. In particular, an incomplete frame can indicate either a torn append or a corrupted earlier length: its offset is not authorization to truncate. Restore from a verified backup and investigate before modifying original bytes. Existing conservative startup handling of incomplete tails is unchanged.

Wave 49 adds a separate, explicitly mutating `block-log-repair-tail` command for a stopped node. It repeats the inspection under the directory lock, requires the exact reported offset, accepts only a structurally incomplete or all-zero suffix, and durably copies the removed bytes to a new backup before truncation. It refuses a complete nonzero corrupt frame rather than guessing. This narrow recovery path does not add per-frame checksums, repair mid-log corruption or decide what to do with a complete frame that fails consensus replay. KS-07 remains partial.
