...
> **Green again 2026-09-05 (finding SC1-advisories).** Measured 2026-09-04, `cargo audit --deny warnings` failed on four advisories: **RUSTSEC-2026-0220** (ruint), **RUSTSEC-2026-0221** (event-listener), **RUSTSEC-2026-0253** (lru), **RUSTSEC-2026-0258** (h2). Three were closed by in-semver `Cargo.lock` bumps, not ignore entries: **h2 0.4.13 → 0.4.19** (fix ≥ 0.4.16), **ruint 1.19.0 → 1.20.0**, **event-listener 5.4.1 → 5.4.2**; the **yanked chacha20 0.10.1** was re-pinned to **0.10.2** in the same pass. The lru advisory is the one residual whose fix (lru ≥ 0.18.2) is **unreachable** — `sp1-prover 4.2.1` pins `lru ^0.12.4` — so it carries a **named, dated, expiring exception**: `RUSTSEC-2026-0253` in `audit.toml`/`.cargo/audit.toml` (host-side proving only; the UB needs a panicking `Drop`/`Hash` impl inside the cache), and a `[[IgnoredVulns]]` entry in `osv-scanner.toml` with `ignoreUntil = 2026-12-01`, after which the blocking osv-scanner job goes red again. **REMOVE all three mirror entries the moment SP1 repins lru ≥ 0.18.2.** (It was in `deny.toml` too until the deep audit of 2026-09-16, LD-02: cargo-deny checks the graph the root workspace *resolves*, where the SP1 stack has not been reachable since `crates/coherence-prover` left the workspace, so that entry — and nine others — matched nothing and were pruned from `deny.toml`; they stay in `.cargo/audit.toml`, which also governs the `coherence-prover/{script,service}` lockfile scans where they do match.)
...
- **`cargo deny check bans` (finding N-5) — was FAILING, now `bans ok`.**
  ...
  **Retired 2026-09-16 (deep audit LD-02):** `windows-sys@0.60.2` has since
  become one of the root lock's unreachable leftovers (`cargo tree --target
  all -i windows-sys@0.60.2` prints nothing), cargo-deny reported the rule
  as `unmatched-skip-root`, and `bans` is `ok` without it, so the
  `skip-tree` rule was removed under the same rule as the advisory ignores:
  an entry that matches nothing in the graph `deny.toml` governs is pruned.
  A future `windows-targets` bump that brings two roots back turns `bans`
  red, and the rule is re-added against the version that is actually there.
