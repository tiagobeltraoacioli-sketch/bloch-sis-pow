...
/// Resident set size and its high-water mark, in KiB, from
/// `/proc/self/status`.
///
/// Instrumentation only, and it exists because peak and steady state turned
/// out to be different regimes on this workload: RSS climbs while the block
/// map fills, then FALLS before replay ends, so `/usr/bin/time -v`'s maximum
/// (which is `VmHWM`) answers a different question from "how much is resident
/// once the chain is loaded". The memory wall is charged by the second, and
/// nothing was reporting it.
///
/// Zero on non-Linux, where the fleet does not run.
fn rss_kib() -> (u64, u64) {
    #[cfg(target_os = "linux")]
    {
        let mut rss = 0;
        ...
        if let Ok(s) = std::fs::read_to_string("/proc/self/status") {
            for line in s.lines() {
                ...
                match it.next() {
                    Some("VmRSS:") => rss = it.next().and_then(|v| v.parse().ok()).unwrap_or(0),
                    Some("VmHWM:") => hwm = it.next().and_then(|v| v.parse().ok()).unwrap_or(0),
                    _ => {}
                }
            }
        }
...
pub fn run(cfg: Config) -> io::Result<()> {
    ...
    if n_logged > 0 {
        ...
        // Steady state, not the high-water mark. See `rss_kib`.
        let (rss, hwm) = rss_kib();
        println!(
            "memory at end of replay: RSS {rss} kB ({:.1} MiB), peak {hwm} kB ({:.1} MiB), \
             {} blocks resident",
            rss as f64 / 1024.0,
            hwm as f64 / 1024.0,
            engine.blocks.len(),
