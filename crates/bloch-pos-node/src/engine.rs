...
/// or what it is reading back out of its own log.
///
...
impl Source {
    /// Whether an envelope from this source is bounded by this node's WALL
    /// CLOCK (`MAX_FUTURE_SLOTS`, `FUTURE_SLOT_TOLERANCE`).
    ///
    /// Only gossip is. Both clock rules exist to bound what an UNTRUSTED
    /// `slot` off the wire can make this node walk or store; neither the
    /// producer's own proposal nor a block read back out of its own log is
    /// untrusted, and neither has any business depending on the wall clock.
    ///
    /// This is written as an allow-list — one source is bounded, everything
    /// else is not — so that adding a source cannot silently subject it to a
...
impl Engine {
    ...
    ///   does not decode, the slot is one no honest node could be gossiping
    ///   (0, or past [`MAX_FUTURE_SLOTS`]), or the proposer signature is a
    ///   forgery under a key this node has registered.
    ...
    /// - `Ignore` — no opinion, and no relay: a duplicate, a block held under
    ///   an identity or a parent this node cannot check yet, or one refused by
    ///   a purely local clock policy ([`FUTURE_SLOT_TOLERANCE`]). None of
    ///   those is a violation the forwarding peer can be charged for.
    ///
    ...
    ///
    /// The path a block this node produced itself takes: there is no peer to
    /// report to, so `Source::Local` and no verdict.
    fn ingest_from(&mut self, env: BlockEnvelope, src: Source) {
        let _ = self.ingest_from_judged(env, src);
    }

    /// Ingest a block read back out of this node's own log during boot replay.
    ///
    /// **This exists so that replay's exemption from the wall-clock rules is
    /// stated, not inherited.** Replay used to call [`Self::ingest`], which
    /// routes to `Source::Gossip` — so a replayed block was judged against
    /// this node's clock by `MAX_FUTURE_SLOTS` exactly as a stranger's block
    /// is. What kept that from refusing a node its own history was an
    /// accident of ordering: `live` is only set AFTER the replay loop, and
    /// the tighter `FUTURE_SLOT_TOLERANCE` is the only check `live` guards.
    /// The horizon was never guarded by it. Any future edit to the live-flag
    ...
    /// grew_registry))` when the block was stored: `id` so the caller can
    /// release what was waiting on it, and `grew_registry` when the block
    /// carried a `Deposit` and therefore may have registered a validator
    /// index some parked block is waiting to be checkable under.
    ///
    /// The order of the checks is load-bearing. Everything that costs only an
    /// integer comparison or a hash of bytes already in hand runs BEFORE the
    /// hybrid signature verify, and every one of them runs before
    /// `self.blocks.insert` — so neither a fork-choice input nor a
    /// post-quantum verification is something an unauthenticated peer can
    /// spend on this node's behalf.
    fn ingest_one(
        &mut self,
        env: BlockEnvelope,
        src: Source,
    ...
    fn ingest_one_deferred(
        ...
        // past by construction, and the horizon above is what bounds them.
        //
        // `Ignore`, not `Reject`: this tolerance is orders of magnitude
        // tighter than [`MAX_FUTURE_SLOTS`] and a peer one slot ahead of this
        // node's clock has proved nothing about itself.
    ...
    fn on_transaction_from_canonical<R>(
        ...
        // signature propagating. What this does NOT catch — an input that
        // does not exist or is not owned by the key, conservation at the
        // current base fee — `admission::check_transfer` above refuses
        // before any signature is paid for; the proposer's probe loop still
        // drops whatever slips past both, which is why the proposer's guard
        // is the one that carries liveness and these only reduce waste.
        // Checks that do not trust each other. Gossip
        // (`NetEvent::Transaction`) and RPC both land in this one call, so
        // one call site carries the whole decision.
            ...
            RpcRequest::SendRawTransaction(tx) => {
                ...
                match self.on_transaction_from_canonical(
                    ...
                    Err(Refusal::AtCapacity) => Err(RpcError::new(
                        rpc::MEMPOOL_FULL,
                        format!(
                            "mempool is at capacity ({MEMPOOL_MAX} entries or {} bytes); retry \
                             later — the transaction was not judged invalid",
                            admission::MAX_MEMPOOL_BYTES,
                        ),
                    )),
                    //
...
mod doppelganger_tests {
    ...
    /// pre-restart instance could have produced); the attestation replayed
    /// by `release_held_turn`, signed for the observation start itself, must.
    #[test]
    fn a_held_then_released_attestation_signed_during_observation_halts() {
        ...
        assert!(
            ...
            "the block alone (signed for slot {boot}, before observation) must not halt"
        );
        // The landed block only queues its waiters; the slot loop replays
        // them in bounded turns. Drain the queue the way the loop does.
        while me.release_held_turn() {}
        assert_eq!(me.att_pool.pending_len(), 0, "fixture: the held attestation was replayed");
        assert!(
            me.doppelganger_halted,
            "the released attestation, signed for slot {duty} at the observation start, is a \
             live duplicate and must halt"
        );
    }
...
/// The mempool door and the packer after the 2026-09-16 audit, pinned
/// against the maintainer's remediation: EN-01 (`pack_transactions` skips
/// an entry that does not fit instead of stopping; the door refuses an
/// over-cap transfer through `admission::check_transfer`), EN-02 / EN-06 /
/// NET-02 (a duplicated, unknown or unowned input is refused before any
/// signature is verified), EN-04 (the per-source cap is answered by the
/// pool's own counters and holds against a fully backed population).
#[cfg(test)]
mod mempool_door_tests {
    use super::transfer_v2_end_to_end::{
        engine_at_wall_epoch, sweep_fixture, sweep_fixture_declaring,
    };
    use super::*;
    use bloch_pos_committee::params::TRANSFER_WITNESS_DEDUP_ACTIVATION_EPOCH as V2_FLAG_DAY;
    use bloch_pos_committee::state_root::EutxoEntry;
    ...
    /// under one key of `pubkey`'s choosing and at `tip`. Handed straight to
    /// the packer by the test that needs a population, never admitted.
    ...
    /// and the selection is empty. Exercised on `pack_transactions` itself:
    /// `select_transactions` first drops what the head state does not back,
    /// which is a different guard and would hide this one.
    #[test]
    fn an_over_cap_entry_at_the_top_of_the_order_no_longer_empties_the_selection() {
        ...
        let mut population: Vec<(Vec<u8>, PosTransaction)> =
            vec![(attacker.canonical_bytes(), attacker.clone())];
        for i in 0..10u32 {
            let honest = filler(i, 0xBB, 0);
            population.push((honest.canonical_bytes(), honest));
        }
        let candidates: Vec<(&Vec<u8>, &PosTransaction)> =
            population.iter().map(|(key, tx)| (key, tx)).collect();
        assert_eq!(candidates.len(), 11, "fixture: eleven distinct entries");

        let selected = Engine::pack_transactions(epoch, candidates);
    ...
    fn a_transfer_larger_than_the_block_byte_cap_is_refused_at_the_door() {
        ...
        assert!(err.reason().contains("block byte budget"), "refused for its size: {}", err.reason());
        ...
        let (entries, declared) = sweep_fixture_declaring(4, cap + 1);
    ...
    /// refused by consensus before any signature; now by the door too. The
    /// declared size leaves room for the extra input, so the refusal is for
    /// the repeat and not for under-declaration.
    #[test]
    fn a_transfer_spending_the_same_output_twice_is_refused_at_the_door() {
        let (entries, mut tx) = sweep_fixture_declaring(4, 12_000);
        ...
        assert!(err.reason().contains("repeats an input"), "{}", err.reason());
        assert!(node.mempool.is_empty());
    }

    /// **EN-02 / NET-02: `UnknownInput`.** An outpoint the head state does
    /// not hold cannot apply: refused, and — unlike a bad signature — as a
    /// state-dependent verdict, since the parent may still land.
    #[test]
    fn a_transfer_spending_an_output_the_chain_does_not_have_is_refused_at_the_door() {
        ...
        assert!(err.reason().contains("absent from committed state"), "{}", err.reason());
        assert!(matches!(err, Refusal::StateDependent(_)), "a missing parent is not a bad transaction");
        assert!(node.mempool.is_empty());
    }
    ...
    /// carried key, the same transfer gets past the ownership check and is
    /// refused later, for a different reason.
    #[test]
    fn a_transfer_whose_key_does_not_own_the_output_is_refused_at_the_door() {
        ...
        assert!(err.reason().contains("does not own its input"), "{}", err.reason());
        assert!(node.mempool.is_empty());

        // Declared above its (tiny) encoding so the size rules stay quiet
        // and the ownership rule is the first one that can speak.
        ...
        let owned: [u8; 32] = Sha3_256::digest([7u8; 16]).into();
        let mut node = engine_at_wall_epoch(V2_FLAG_DAY + 1, &[coin(owned)]);
        let err = node.on_transaction(v1).expect_err("control: still unpayable and unsigned");
        assert!(
            !err.reason().contains("own its input"),
            "owned outpoint: past the ownership check, refused for another reason: {}",
            err.reason()
        );
        assert!(node.mempool.is_empty());
    }

    /// The per-source cap (R7 M6) holds against a population capacity
    /// planning cannot reclaim: 64 pending transfers under the fixture's
    /// key, every one backed by the head state, then a 65th — refused as
    /// `TooManyFromSource`, with nothing evicted to make room for it.
    #[test]
    fn the_per_source_cap_is_still_enforced_against_a_backed_population() {
        ...
        for i in 1..=MEMPOOL_MAX_PER_SOURCE as u64 {
            // Distinct bytes (the declared size is inside the signing root),
            // same source, same backing outputs.
            let (_, pending) = sweep_fixture_declaring(4, 12_000 + i);
            node.mempool.insert(pending.canonical_bytes(), pending);
        }
        assert_eq!(node.mempool.source_count(&source), MEMPOOL_MAX_PER_SOURCE);

        assert_eq!(node.on_transaction(tx), Err(Refusal::TooManyFromSource));
        assert_eq!(
            node.mempool.len(),
            MEMPOOL_MAX_PER_SOURCE,
            "nothing reclaimed: every pending entry is backed"
        );
        assert_eq!(node.mempool.source_count(&source), MEMPOOL_MAX_PER_SOURCE);
    }
}
...
mod legacy_exit_alert_tests {
    ...
    /// A block from ANOTHER proposer carrying a legacy exit. This node's own
    /// packer never proposes one (`pack_transactions`, TX-01), so the body
    /// is written the way a hostile proposer would write it: a real proposal
    /// from a second engine on the same key, its body replaced, its roots
    /// recomputed over the genesis state and its header re-signed.
    #[test]
    fn an_applied_legacy_exit_is_counted_and_alerted() {
        ...
        let (mut other, _dir) = perf_support::proposing_engine();
        let _clock = validator_lifecycle::clock_at(1);
        other.wall_slot = 1;
        other.propose(1);
        let mut block = other.blocks[other.head_id().as_bytes()].clone();
        assert_eq!(block.header.slot, 1, "fixture: the other instance must propose");
        ...
        me.canonical = BTreeSet::from([*me.manifest.genesis_id().as_bytes()]);
        me.wall_slot = 1;
        assert_eq!(me.legacy_exits_applied, 0);

        let txs = vec![exit.clone()];
        block.body.transactions = vec![exit.canonical_bytes()];
        block.header.body_root = derive::body_root(&block.body.transactions);
        let probe = ProposalEnvelope { header: block.header.clone(), proposer_sig: Vec::new() };
        let post = me
            .tr_probe
            .compute_post_state(&me.state, &probe, &block.body.attestations, &txs)
            .expect("fixture: the transition applies a legacy exit before its flag day");
        block.header.state_root = post.state_root();
        block.proposer_sig = other
            .keys
            .as_ref()
            .expect("the proposing fixture holds a keystore")
            .sign(&block.header.proposal_signing_root());
        let id = block.block_id();

        me.ingest(block);
        assert_eq!(me.head_id(), id, "fixture: the block must be adopted");
        assert_eq!(me.legacy_exits_applied, 1, "the applied exit is counted");
        assert_ne!(
            me.state.validator_record(0).expect("validator 0").exit_epoch,
            u64::MAX,
...
mod slot_horizon {
    ...
    /// block INSIDE both clock rules is not refused BY them — it reaches the
    /// gates that come after. Without this, `ingest_judged` returning `Reject`
    /// unconditionally would pass.
    ///
    /// The witness is the ORPHAN POOL, and it is the right one precisely
    /// because parking happens strictly behind both clock rules AND behind
    /// H8's proposer-authentication gate. This fixture's envelope names a
    /// proposer index the fixture state never registered, so an envelope that
    /// reaches `orphans` is one that BOTH the horizon and the tolerance let
    /// through. `rejected_future == 0` says the same thing from the other
    /// side, and `blocks` staying empty says H8's rule is still in force.
    #[test]
    fn a_block_inside_the_horizon_reaches_the_gates_behind_it() {
        let mut e = engine_at_wall_slot(10);
        let wall = e.wall_slot();
        // Inside the horizon (64) AND inside the tolerance (8): the clock has
        // no answer for this block, so a later rule must be the one that does.
        let env = envelope_at_slot(wall + FUTURE_SLOT_TOLERANCE);
        let id = *env.block_id().as_bytes();

        let verdict = e.ingest_judged(env);
        assert_eq!(
            e.rejected_future, 0,
            "a block inside the tolerance must not be charged to the clock",
        );
        assert_eq!(
            e.orphans.len(),
            1,
            "it must have reached the gates behind the clock — that is what proves neither slot rule turned it away",
        );
        assert_eq!(
            verdict,
            Verdict::Ignore,
            "held under an identity this node cannot check is no charge against the peer that relayed it",
        );
        assert!(
            !e.blocks.contains_key(&id),
            "and an unauthenticated block still never reaches `blocks` (H8)",
        );
    }
    ...
    /// fleet, routinely — must keep judging current blocks on their merits,
    /// or the rule would quietly stop such a node from ever catching up.
    #[test]
    fn a_node_far_behind_its_own_chain_still_judges_current_blocks_on_merit() {
        ...
        let id = *env.block_id().as_bytes();
        let verdict = e.ingest_judged(env);
        assert_eq!(e.rejected_future, 0, "a current block is never past any clock rule");
        assert_eq!(
            e.orphans.len(),
            1,
            "however far behind the chain is, a current block still gets past both slot rules to the gates behind them",
        );
        assert_eq!(verdict, Verdict::Ignore);
        assert!(!e.blocks.contains_key(&id));
    }

    /// **The horizon is not shadowed by the tighter tolerance.**
    ///
    /// `FUTURE_SLOT_TOLERANCE` (8 slots) is stricter than `MAX_FUTURE_SLOTS`
    /// (64) and would answer first — but only while the node is `live`. During
    /// boot and sync it is skipped entirely, and this horizon is then the only
    /// thing standing between a gossiped `u64::MAX` and `epoch_of` walking it.
    ///
    /// Delete the horizon check and this fails: the envelope lands in the
    /// orphan pool instead of being turned away.
    #[test]
    fn the_horizon_still_bounds_a_gossiped_slot_while_the_node_is_not_live() {
        let mut e = engine_at_wall_slot(10);
        e.live = false;
        let env = envelope_at_slot(u64::MAX);
        let id = *env.block_id().as_bytes();
        ...
        assert_eq!(
            e.ingest_judged(env),
            Verdict::Reject,
            "the tolerance is off during sync; the horizon must still answer",
        );
        assert_eq!(e.rejected_future, 0, "and it is the horizon answering, not the tolerance");
        assert!(!e.blocks.contains_key(&id));
        assert!(
            e.orphans.is_empty(),
            "a slot past the horizon must not even reach the orphan pool",
        );
    }
...
mod ingest_admission_tests {
    ...
    fn a_block_far_ahead_of_the_wall_clock_is_refused() {
        ...
        // loaded the box is; forty makes it a test of the rule.
        //
        // Forty and not ten thousand, because C1's `MAX_FUTURE_SLOTS` horizon
        // now sits in front of this check and would answer first for a slot
        // that far out. Staying inside the horizon is what keeps THIS test
        // about the tolerance — `rejected_future` is incremented on no other
        // path, so it also proves the horizon did not shadow the tolerance.
        let far = repointed(
            &engine,
            &template,
            [0x9A; 32],
            engine.wall_slot() + FUTURE_SLOT_TOLERANCE + 40,
        );
        engine.ingest(far);
    ...
    /// opposite answers:
    ///
    /// - down [`Engine::ingest_replay`] — the boot path, reading this node's
    ///   own committed log — it is applied, however far its slot sits past
    ///   this box's wall clock;
    /// - arriving over gossip on a live node it is `Reject`ed by
    ///   `MAX_FUTURE_SLOTS` and never reaches `blocks`, which is C1's whole
    ///   point and is not weakened by the exemption above it.
    ///
    /// The slot is past the HORIZON, not merely past the tolerance, because
    /// the horizon is the check `self.live` never guarded. Boot replay used to
    /// route through `ingest` -> `Source::Gossip`, so a replayed block was
    /// judged against this node's clock exactly as a stranger's is; what kept
    /// that from refusing a node its own finalized history was only that a
    ...
    fn boot_replay_applies_a_block_the_same_node_would_reject_from_gossip() {
        ...
        // Past `MAX_FUTURE_SLOTS`, with margin: the fixture's genesis is
        // `now` at a 1 s cadence and hybrid signing in a debug build takes
        // real seconds, so the wall slot moves underneath this test. 100
        // slots of headroom keeps the block past the horizon at every point
        // below rather than making the assertion depend on load.
        let far = engine.wall_slot() + MAX_FUTURE_SLOTS + 100;
        engine.propose(far);
        assert_eq!(
            engine.head_slot_now(),
            far,
            "the fixture must have produced the block this test is about"
        );
        let env = engine
            .blocks
            .get(engine.head_id().as_bytes())
        ...
        // Give the chain back and forget the envelope, so the SAME bytes can
        // be delivered again down a different path. Both `blocks` and
        // `canonical` have to let go, or `ingest_one`'s duplicate check
        // answers first and neither half proves anything.
        let forget = |engine: &mut Engine| {
            assert!(
                engine.do_reorg(genesis, Vec::new()),
                "giving the chain back must succeed"
            );
            engine.blocks.remove(&id);
            assert_eq!(*engine.head_id().as_bytes(), genesis);
            assert!(!engine.canonical.contains(&id), "and it is no longer canonical");
        };
        forget(&mut engine);
        ...
        assert_eq!(
            engine.head_slot_now(),
            far,
            "a node must apply a block out of its OWN log however far that \
             block's slot sits past this box's wall clock — refusing here is a \
             node that cannot restart"
        );
        assert!(engine.blocks.contains_key(&id), "and it must have been stored");
        assert_eq!(
            engine.rejected_future, 0,
            "replay is not charged to either clock rule"
        );

        // ── Half two: the same bytes, over the wire, on a live node.
        forget(&mut engine);
        engine.live = true;
        ...
        assert_eq!(
            verdict,
            Verdict::Reject,
            "the exemption is for this node's own log and nothing else — a \
             gossiped slot past the horizon is still provably refusable"
        );
        assert!(
            !engine.blocks.contains_key(&id),
            "and it must never reach `blocks` (C1)"
        );
        assert_eq!(
            engine.head_slot_now(),
            0,
