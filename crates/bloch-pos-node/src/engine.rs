...
impl Engine {
    ...
    fn serve_rpc(&mut self, req: RpcRequest) -> RpcResult {
        match req {
            // Touches no consensus state at all: every field is stamped at
            // compile time. See [`rpc::node_version_json`] for what the answer
            // does and does not prove about the running node.
...
mod transfer_v2_end_to_end {
    ...
    /// `getnodeversion` is served by the engine, and costs it no state.
    ///
    /// Registering a method in the dispatcher without an arm in `serve_rpc`
    /// is a compile error (the match is exhaustive), so what this adds is the
    /// end of the chain: the method a client calls actually returns the stamp.
    #[test]
    fn getnodeversion_is_answered_by_the_engine() {
        let (entries, _tx) = sweep_fixture(1);
        let mut node = engine_at_wall_epoch(V2_FLAG_DAY + 1, &entries);
        let RpcResult::Ok(v) = node.serve_rpc(RpcRequest::NodeVersion) else {
            panic!("getnodeversion must answer");
        };
        assert_eq!(v.get("build").unwrap().as_str(), Some(env!("BLOCH_BUILD_VERSION")));
        assert!(!v.get("commit").unwrap().as_str().unwrap().is_empty());
        let wire = on_the_wire(Ok(v));
        assert!(wire.contains(r#""commit""#) && wire.contains(r#""dirty""#), "{wire}");
    }
