...
impl Net {
    ...
    /// `prov` says whether this frame is this node's own work or a forward of
    /// someone else's; see [`Provenance`]. Returns false when the frame reached
    /// NO peer at all. The transports count and log the loss themselves, so a
    /// caller with nothing to add may ignore the result — a caller that can name
    /// what was lost (the proposer knows its slot) should say so.
...
//
// The inbound half of this file now sheds loudly (see [`EngineQueue`]). The
// OUTBOUND half did not: every publish was `let _ = p.send(...)` or
// `let _ = self.cmd.send(...)`, a discarded `Result` on the exact path a
// validator uses to announce the block it just produced and signed.
//
// The two are not the same problem and do not have the same answer. A shed
// inbound frame is one this node chose not to look at, and the network still
// has it. A dropped OUTBOUND frame that this node originated exists nowhere
// else in the world: the slot is spent, the proposal is gone, and the only
// evidence was an empty slot in someone else's log.
...
/// frame this node fails to forward is still in the network — its author
/// published it and other peers carry it. A frame this node ORIGINATED (the
/// block it built, the attestation it cast, the history it is asking for) is
/// lost outright.
///
/// It is a parameter and not a guess made inside the transport because the
/// transport cannot tell: the same `FRAME_TX` bytes reach `broadcast` both from
/// a peer's gossip and from this node's own RPC. Only the caller knows.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum Provenance {
    /// Produced by this node. EVERY failure is logged, individually.
    Originated,
    /// Forwarding someone else's, or a frame whose author this node cannot
    /// determine. Failures are counted, and logged on a doubling schedule.
    Relayed,
}

/// Frames this node failed to put on the wire, by provenance.
///
/// Counters and not log lines alone, because an operator needs to be able to
/// ask "is this node publishing?" and get a number, and because a test can
/// assert on a number and cannot assert on `eprintln!`.
...
impl BroadcastDrops {
    ...
    /// Frames refused before provenance mattered — an empty frame with no type
    /// byte. Its own counter because it is a bug in this process, not a peer.
    ...
    pub(crate) fn note_malformed(&self) {
        ...
        if n.is_power_of_two() {
            eprintln!("net: refused an EMPTY frame with no type byte — {n} so far this run");
        }
    }
    ...
    pub(crate) fn lost(&self, prov: Provenance, frame: &[u8], reason: &str, refused: usize) {
        ...
        match prov {
            Provenance::Originated => {
                ...
                // EVERY one of these, always. This line is the whole difference
                // between "we lost a proposal" and an empty slot nobody can
                // explain.
                eprintln!(
            ...
            Provenance::Relayed => {
                ...
                // A relay drop is an ordinary consequence of a slow peer and a
                // line per event would be the whole log during a flood — but
                // going silent is the defect this work exists to remove.
                // Doubling keeps the volume flat per order of magnitude while
                // the count stays exact. No clock is involved, so no clock can
                // fail this into silence.
                if n.is_power_of_two() {
                    eprintln!(
    ...
    pub(crate) fn partial(&self, prov: Provenance, frame: &[u8], accepted: usize, refused: usize) {
        ...
        // "our block reached 3 of 60 peers" is the exact shape of the
        // 2026-08-21 incident, and nothing in this node could see it.
        if n.is_power_of_two() {
            eprintln!(
...
impl DevnetMesh {
    ...
    /// `let _ = p.send(...)`: a node whose peers had all gone away — or which
    /// had none — published its own signed block into a closed channel and said
    /// nothing at all. The outcome is now counted always, and logged every time
    /// for a frame this node originated.
    pub fn broadcast(&self, frame: Vec<u8>, prov: Provenance) -> bool {
        ...
        for p in &self.peers {
            match p.send(frame.clone()) {
                Ok(()) => accepted += 1,
                // This peer's dialer thread has exited. It respawns on
                // reconnect, so the sender stays in the list; this frame is
                // simply gone.
                Err(_) => refused += 1,
            }
        }
        ...
        if let Ok(mut inbound) = self.inbound.lock() {
            inbound.retain(|p| match p.try_send(frame.clone()) {
                ...
                // A full queue is a peer not reading fast enough. It keeps its
                // slot — it is alive — but this frame is lost to it, and that
                // is precisely the loss the discarded `Result` could not show.
...
/// Tests for the OUTBOUND half: what happens when a frame this node is trying
/// to publish does not reach anyone.
///
/// The inbound quota above decides what this node refuses to look at. These
/// decide whether this node can tell that its own block never left.
#[cfg(test)]
mod broadcast_tests {
    use super::*;

    /// A mesh with `live_out` dialed peers whose readers are held open,
    /// `dead_out` dialed peers whose readers have been dropped, and `full_in`
    /// inbound peers whose queues are already full.
    ///
    /// The live receivers are returned and must be held by the caller: dropping
    /// one turns a live peer into a dead one, which is exactly the failure
    /// these tests induce on purpose and must not induce by accident.
    fn mesh(
        live_out: usize,
        dead_out: usize,
        full_in: usize,
    ) -> (DevnetMesh, Vec<Receiver<Vec<u8>>>, Vec<mpsc::Receiver<Vec<u8>>>) {
        let mut peers = Vec::new();
        ...
        for _ in 0..dead_out {
            ...
            drop(rx); // the writer thread for this peer has exited
            peers.push(tx);
        }
        let mut inbound = Vec::new();
        let mut keep_in = Vec::new();
        for _ in 0..full_in {
            // Depth 1, already occupied: a peer that is connected and not
            // reading, which is the shape a stalled validator presents.
            let (tx, rx) = mpsc::sync_channel::<Vec<u8>>(1);
            tx.try_send(vec![0u8]).expect("depth 1 takes the first");
            inbound.push(tx);
            keep_in.push(rx);
        }
        let m = DevnetMesh {
            ...
            drops: Arc::new(BroadcastDrops::default()),
        };
        (m, keep, keep_in)
    }
    ...
    fn frame() -> Vec<u8> {
        let mut f = vec![FRAME_BLOCK];
        f.extend_from_slice(&[9u8; 128]);
        f
    }

    /// A frame that reached NOBODY must be reported to the caller and counted,
    /// and our own must be told apart from a relay.
    ///
    /// This is the `let _ = p.send(frame.clone())` that part (B) exists to
    /// remove: before it, every case below returned `()` and moved no counter.
    #[test]
    fn a_frame_that_reaches_nobody_is_reported_and_counted() {
        // CONTROL first, so a mesh that dropped everything could not pass this
        // test by accident.
        let (m, _keep, _in) = mesh(1, 0, 0);
        assert!(m.broadcast(frame(), Provenance::Originated), "a live peer must take the frame");
        assert_eq!(m.drops().originated_lost(), 0, "a delivered frame counted as lost");
        assert_eq!(m.drops().originated_partial(), 0, "a fully delivered frame counted as partial");

        // No peers at all: a proposer publishing into the void.
        let (m, _keep, _in) = mesh(0, 0, 0);
        assert!(!m.broadcast(frame(), Provenance::Originated), "a mesh with no peers reported success");
        assert_eq!(m.drops().originated_lost(), 1);

        // Every peer gone: the same loss, by a different route.
        let (m, _keep, _in) = mesh(0, 3, 0);
        assert!(!m.broadcast(frame(), Provenance::Originated), "three dead peers reported success");
        assert_eq!(m.drops().originated_lost(), 1);

        // A relay drop is counted too — on its own counter, never on ours.
        let (m, _keep, _in) = mesh(0, 1, 0);
        assert!(!m.broadcast(frame(), Provenance::Relayed));
        assert_eq!(m.drops().relayed_lost(), 1, "a lost relay was not counted");
        assert_eq!(m.drops().originated_lost(), 0, "a relay drop was charged to origination");
    }

    /// Reaching SOME peers is not the same as reaching none, and it is the
    /// shape of the 2026-08-21 incident — a proposal that reached 3 of 60.
    #[test]
    fn a_frame_that_reaches_only_some_peers_is_counted_partial() {
        let (m, _keep, _in) = mesh(1, 1, 0);
        assert!(
            m.broadcast(frame(), Provenance::Originated),
            "one live peer means the frame did leave this node"
        );
        assert_eq!(m.drops().originated_lost(), 0, "a partial delivery is not a total loss");
        assert_eq!(m.drops().originated_partial(), 1, "a partial delivery was not counted");
    }

    /// A full inbound queue is a lost frame and a KEPT peer: the connection is
    /// alive and must not be pruned, but the frame is gone and must be counted.
    #[test]
    fn a_full_inbound_queue_loses_the_frame_without_losing_the_peer() {
        let (m, _keep, _keep_in) = mesh(0, 0, 1);
        assert!(
            !m.broadcast(frame(), Provenance::Originated),
            "the only peer's queue was full, so the frame reached nobody"
        );
        assert_eq!(m.drops().originated_lost(), 1);
        assert_eq!(
            m.inbound.lock().expect("no other thread holds this").len(),
            1,
            "a peer that is merely behind must keep its slot"
        );
    }

    /// An inbound peer whose reader is gone is pruned, and the loss is counted.
    #[test]
    fn a_disconnected_inbound_peer_is_pruned_and_its_loss_counted() {
        let (tx, rx) = mpsc::sync_channel::<Vec<u8>>(4);
        drop(rx);
        let m = DevnetMesh {
            peers: Vec::new(),
            inbound: Arc::new(Mutex::new(vec![tx])),
            drops: Arc::new(BroadcastDrops::default()),
        };
        assert!(!m.broadcast(frame(), Provenance::Originated));
        assert_eq!(m.drops().originated_lost(), 1);
        assert!(
            m.inbound.lock().expect("no other thread holds this").is_empty(),
            "a disconnected peer must be pruned, or the list grows with every reconnect"
        );
    }
}
