...
//!
//! **This is the binary the live chain runs.** Genesis-4 has been producing
//! blocks under proof of stake since 21:31:19 UTC on 2026-08-13, when
//! Genesis-3 (proof of work) stopped permanently at height 39,918. The
//! Genesis-4 cohort is **64 validator processes**, all operated by a single
//! entity; they produce blocks, attest, justify and finalize with real
//! ML-DSA-65 ‖ Falcon-1024 hybrid signatures over append-only block-log
//! persistence (restart = deterministic replay).
//!
//! Since 2026-08-13 it also carries a libp2p network layer (`p2p`): gossipsub
//! on Genesis-4-only protocol ids, carrying the Genesis-3 mesh fixes, with
//! `gossip.rs` wired as attestation admission control and a directed,
//! paginated block-sync path. A node started with an **empty data dir** syncs
//! from genesis and validates every block itself — no datadir is ever
//! donated. `--transport devnet` selects the original TCP mesh and **is still
//! the default, and is what the live fleet runs**: a point-to-point TCP full
//! mesh with a fixed peer list, no discovery and no authentication, which is
//! why a third party cannot yet join the network.
//!
...
//! exchange, no slashing-evidence pipeline, and no checkpoint-sync **state
//! download** (the weak-subjectivity boot gate itself IS wired — `ws_boot`).
//! `Deposit` and `Delegate` are refused at every node's mempool because
//! bonding is not yet funded from the UTXO set (`engine::admissible`), so
//! there is no permissionless path to becoming a validator today; transfers
//! are open. JSON-RPC 2.0 over HTTP **is** served (`rpc`), and the mainnet
//! genesis manifest **is** built here (`genesis-mainnet`). See the module
//! docs of `engine`, `net`, `p2p`, `store`, `ws_boot` for each limitation at
//! its site.
//!
//! ## The security question, stated once
//!
//! It is not hashrate — Genesis-4 has no mining. It is **concentration**: all
//! 64 validators are run by one entity, 93.94% of the carryover sits at a
...
//! holder can outvote every other. See
//! `bloch_pos_committee::tokenomics_v4` for the pinned constants.
//!
//! ## Key hygiene
//!
//! `keygen` produces **throwaway, non-production keys only** (never printed,
//! 0600 files); it is a test/bring-up tool, not the path a live validator key
//! is meant to come from. Production keys are generated by a human on an
//! air-gapped machine per BLOCH-GENESIS-KEYS.md rule zero — never by this tool
//! in an observable session.
//!
//! ## Ownership
//!
        ...
        "{NAME} {VERSION} — Bloch Genesis-4 Proof-of-Stake node (devnet stage)\n\
         ...
         USAGE:\n\
           ...
           bloch-pos keygen --dir <dir> --index <i>\n\
               Generate a THROWAWAY validator keystore (hybrid\n\
               ML-DSA-65‖Falcon-1024 + RANDAO seed) at <dir>/validator.key.\n\
               Test/bring-up only; production keys follow\n\
               BLOCH-GENESIS-KEYS.md and are generated air-gapped.\n\
           ...
           bloch-pos genesis --keys <dir1,dir2,...> --out <file>\n\
               ...
               Build a TEST genesis manifest from the keystores' public\n\
               parts: a validator set and a clock, no carryover and no\n\
               allocations. Slot 0 starts <secs> from now (default 5).\n\
               The LIVE mainnet manifest is built by `genesis-mainnet`.\n\
           bloch-pos genesis-mainnet --cohort <cohort.tsv> --out <file>\n\
                                     [--start-in <secs>]\n\
               Build the Genesis-4 MAINNET manifest: the launch cohort from\n\
               <cohort.tsv> plus the tokenomics-V4 carryover commitment and\n\
               vested allocations. Every field is refused rather than\n\
               defaulted.\n\
           bloch-pos run --data-dir <dir> --genesis <file>\n\
                         [--transport devnet|libp2p]\n\
               devnet (default) is the TCP full mesh, and it is what the\n\
               LIVE fleet runs: a point-to-point full mesh over a fixed peer\n\
               list, with no discovery, no authentication, no admission\n\
               control and no relay logic. That is why a third party cannot\n\
               yet join the network.\n\
               libp2p is the newer stack: gossipsub on Genesis-4-only\n\
         ...
         \n\
             devnet transport:\n\
               ...
               127.0.0.1, which is what a fleet spread across hosts needs.\n\
         ...
         \n\
               ...
               total — before a single balance is admitted. A test\n\
               ...
               *** The RPC has NO AUTHENTICATION. *** No API key, no rate\n\
               ...
               build bakes NO signer arrangement, so --ws-signer-set must\n\
               accompany --ws-checkpoint. With neither flag the genesis\n\
               anchor is the first checkpoint, which is why a fresh network\n\
...
/// `Delegate` are refused at every node's mempool today (`engine::admissible`)
/// because bonding is not yet funded from the UTXO set, so emitting one would
/// only produce a rejection; and evidence cannot travel this way at all — its
/// canonical encoding is one-way (see `PosTransaction::from_canonical_bytes`).
///
...
///
/// The test-only `genesis` command reads keystores, which is exactly what must
...
fn genesis_cmd(args: &[String]) {
    ...
    let manifest = genesis::Manifest {
        ...
        validators,
        cohort: Vec::new(),
        // `genesis` builds TEST manifests: no carried balances, no vested
...
fn run_cmd(args: &[String]) {
    ...
    // `Transport::Devnet` stays the default, and it is what the LIVE
    // Genesis-4 fleet runs: the 64-validator cohort finalized on that
    // transport and still does, and the same command must keep producing the
    // same run. It is a fixed-peer TCP full mesh with no discovery and no
    // authentication — the reason a third party cannot yet join. Opting into
    // the libp2p stack is an explicit act.
