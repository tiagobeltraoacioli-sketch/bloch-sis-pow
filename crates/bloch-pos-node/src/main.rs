...
use bloch_pos_committee::params::{
    ...
    FUNDED_STAKING_ACTIVATION_EPOCH, LEAKED_ROSTER_ACTIVATION_EPOCH,
    LEAK_RECOVERY_ACTIVATION_EPOCH, SIGNED_EXIT_ACTIVATION_EPOCH, SLOTS_PER_EPOCH,
    SLOT_DURATION_SECS, TRANSFER_WITNESS_DEDUP_ACTIVATION_EPOCH, VESTING_LOCK_ACTIVATION_EPOCH,
...
const CONSENSUS_GATES: [(&str, u64); 9] = [
    (
    ...
    // The four the consensus integration of 2026-08-31 introduced, all INERT.
    // Listed in the numbering of the flag-day calendar's §11.1 so the two can
    // be read side by side. They change `gates_digest` — a different gate SET
    // is a different compatibility statement, which is the point — and they
    // leave `knows_gates_through_epoch` at 1400, because none is armed.
    (
        "VESTING_LOCK_ACTIVATION_EPOCH",
        VESTING_LOCK_ACTIVATION_EPOCH,
    ),
    (
