...
//! context.** `keygen` here exists for *throwaway, non-production* validators
//! only — test networks and local bring-up, never the live cohort:
...
impl Keystore {
    /// Generate a throwaway keystore at `dir/validator.key`.
    ///
    /// NON-PRODUCTION ONLY — test networks and local bring-up, never a live
    /// validator. The RANDAO seed comes from the OS entropy pool; the hybrid
    /// keypair from bloch-crypto's keygen. File mode 0600.
...
/// this map.
///
/// The deposit path IS wired in consensus — `PosTransaction::Deposit` and
/// `Delegate` exist, the transition applies them, and `staking.rs` runs the
/// activation queue. What keeps the live set static is a node-side refusal:
/// `engine::admissible` rejects both variants at every node's mempool because
/// bonding is not yet funded from the UTXO set, so a deposit would create
/// stake without spending coins. Until that is closed — a wire-format change
/// needing a flag day — the Genesis-4 set is exactly the 64 validators
/// genesis named, and this map is complete.
#[derive(Clone)]
...
/// Fill `buf` from the OS entropy pool (`/dev/urandom`). Adequate for the
/// throwaway keystores above; a live validator key is never generated here.
