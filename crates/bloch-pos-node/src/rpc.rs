...
///
/// Read from the live chain on 2026-09-01 at epoch 1726, two archivals
/// agreeing on the same head and root: 64 validators, 64 active, every record
...
/// ever been prosecuted. A figure of "48 double-signing validators" has
/// circulated internally: it is **not substantiated** anywhere in this
/// repository, no RPC exposes equivocation history, and the only equivocation
/// this repo actually records is two indices (`deploy/FLAG-DAY-EPOCH-800.md`).
/// Do not quote the number. Nothing above depends on it. `slashing.rs` is
