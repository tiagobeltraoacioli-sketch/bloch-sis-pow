...
fn getchaininfo_reports_slot_epoch_head_root_and_both_checkpoints() {
    ...
    // describes.
    let v = chain_info_json(&st, &head, st.state_root(), 0, Some(0), 12, 2, 3, 0, "devnet", (Some(0), None));
...
fn getbuildinfo_carries_a_bounded_build_environment_fingerprint() {
    ...
    assert!(scope.contains("selected freestanding native include-tree contents"));
