...
//!
//! The claim survived in four places at once — a doc comment, a JSON
//! capability object, a specification and a web page — because prose does not
...
const RETRACTION_SITES: &[(&str, &str)] = &[
    ...
    // The machine-readable half: a client can branch on this instead of prose.
    ("crates/bloch-pos-node/src/rpc.rs", "\"slashing_enforced\""),
    // The published RPC contract.
    ("docs/specs/BLOCH-RPC-STABILITY-V4.md", "no stake on this network can be slashed"),
    // The book the exchange integrates from.
    (
        "docs/integration/BLOCH-GENESIS4-EXCHANGE-INTEGRATION.md",
        "finality here is not backed by a slashing cost",
    ),
    // The document that threatens operators with the same penalty.
    ("docs/VALIDATOR-RUNBOOK.md", "none of \u{a7}14 is enforceable today"),
    // The public block explorer: the block page and the `final` badge tooltip.
    ("apps/explorer/src/pages/G4Block.tsx", "not backed by any slashing penalty"),
    ("apps/explorer/src/components/corroboration.tsx", "no slashing penalty backs it"),
    ...
    match reachability() {
        Reachability::RefusedByConstruction => assert!(
            ...
            "Text promises a slashing penalty that CANNOT BE APPLIED. Tag 0x05 is \
             ...
             This is the claim retracted on 2026-09-01 across rpc.rs, the RPC \
             stability spec, the exchange integration book, the validator runbook \
             and the block explorer. If it is genuinely true again, the retractions \
