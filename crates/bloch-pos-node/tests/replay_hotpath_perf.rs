...
use bloch_pos_committee::state_root::{
    build_state_tree, eutxo_leaf, state_root_with_eutxo_leaves, BaseFeeRecord, CheckpointRecord,
...
fn breakdown() {
    ...
    let t = Instant::now();
    let root_inc = state_root_with_eutxo_leaves(&state(&f, &empty), &leaves);
    ...
    let doomed: Vec<[u8; 32]> = leaves2.keys().take(4).copied().collect();
    for k in &doomed {
        leaves2.remove(k);
    }
    for i in 0..4u32 {
    ...
    let t = Instant::now();
    let root_edited = state_root_with_eutxo_leaves(&state(&f, &empty), &leaves2);
    ...
    println!("  d) state_root_with_eutxo_leaves (warm)        : {:>10.1} ms", ms(t_incremental));
...
/// `state_root_with_eutxo_leaves` path with the kept leaves).
#[test]
#[ignore]
