...
fn breakdown() {
    ...
    let t = Instant::now();
    let root_inc = state_root_with_eutxo_tree(&state(&f, &empty), &Smt::from_leaf_map(&leaves));
    ...
    let t = Instant::now();
    let root_edited = state_root_with_eutxo_tree(&state(&f, &empty), &Smt::from_leaf_map(&leaves2));
