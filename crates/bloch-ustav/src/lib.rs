...
/// Opt-in verifier requiring canonical primitive signature encodings.
///
/// The Ustav host already requires explicit suite-0x0001 envelopes. This type
/// additionally rejects Falcon's alternate zero-padded representation without
/// changing [`BlochVerifier`]'s compatibility policy.
#[derive(Clone, Copy, Debug, Default)]
pub struct CanonicalBlochVerifier;

// Encoding pinned by bloch-crypto's pqcrypto-falcon =0.4.1 dependency and KATs.
const FALCON_PUBLIC_KEY_BYTES: usize = 1793;

fn valid_pq_key(key: &[u8]) -> bool {
...
impl Verifier for BlochVerifier {
    ...
    fn verify_pq(&self, message: &[u8], key: &[u8], signature: &[u8]) -> bool {
        verify_pq_with(message, key, signature, crypto::verify_enveloped)
    }
}

impl Verifier for CanonicalBlochVerifier {
    fn valid_pq_key(&self, key: &[u8]) -> bool {
        valid_pq_key(key)
    }
    ...
    fn verify_pq(&self, message: &[u8], key: &[u8], signature: &[u8]) -> bool {
        verify_pq_with(
            ...
            key,
            signature,
            crypto::verify_enveloped_canonical,
        )
    }
}
