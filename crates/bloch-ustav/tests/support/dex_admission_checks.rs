...
use bloch_pos_committee::transition::{
    native_dex::{pool_batch, pool_wire, State},
    PosTransaction,
};
use bloch_ustav::{
...
pub(super) fn check(anchor: &State, frames: &[&[u8]], final_state: &State) {
    ...
    assert!(pending.build(&journal).is_err());
    assert!(matches!(
        pending.admit(&journal, &vec![0; pool_batch::MAX_BYTES as usize + 1]),
        Err(Error::ResourceLimit)
    ));
    assert!(pending.admit(&journal, frames[1]).is_err()); // depends on the first add
    ...
    assert!(pending
        .admit(&journal, &pool_wire::encode(&forged, &DOMAIN).unwrap())
        .is_err());
    let mut wrong_domain = frames[0].to_vec();
    wrong_domain[10] ^= 1;
    assert!(pending.admit(&journal, &wrong_domain).is_err());
    let mut expired = PendingBatch::new(&journal, 101).unwrap();
    assert!(expired.admit(&journal, frames[0]).is_err());
    ...
    pending.admit(&journal, frames[0]).unwrap();
    assert_eq!(pending.len(), 1);
    assert_eq!(pending.wire_bytes(), frames[0].len() as u64);
    assert!(matches!(
        pending.admit(&journal, frames[0]),
        Err(Error::Duplicate)
    ));
    let first_candidate = pending.build(&journal).unwrap();
    assert!(pending.admit(&journal, &frames[1][..20]).is_err());
    assert_eq!(pending.len(), 1);
    assert_eq!(pending.build(&journal).unwrap(), first_candidate);
    pending.admit(&journal, frames[1]).unwrap();
    let two_candidate = pending.build(&journal).unwrap();
    ...
    assert!(matches!(
        pending.admit(&journal, &pool_wire::encode(&forged, &DOMAIN).unwrap()),
        Err(Error::Batch(pool_batch::Error::Operation { index: 2, .. }))
    ));
    assert_eq!(pending.len(), 2);
    assert_eq!(pending.build(&journal).unwrap(), two_candidate);
    let preview = pending.admit(&journal, frames[2]).unwrap();
    ...
    assert!(matches!(pending.build(&other), Err(Error::StaleParent)));
    drop(other);
    ...
    competing.admit(&journal, frames[0]).unwrap();
    let committed = pending.commit(&mut journal).unwrap();
    ...
    assert!(matches!(pending.commit(&mut journal), Err(Error::Closed)));
    assert!(matches!(
        pending.admit(&journal, frames[0]),
        Err(Error::Closed)
    ));
    assert!(matches!(
        competing.commit(&mut journal),
