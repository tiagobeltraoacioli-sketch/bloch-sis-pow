//! Real PQ atomic LP redemption from sealed BLCH/native reserves; local rehearsal only.
...
use bloch_pos_committee::transition::native_dex::{
    initial_liquidity, remove_liquidity, swap, swap_quote,
};

fn initialized() -> (State, [u8; 32], [u8; 32], ([u8; 32], u32)) {
    let (mut state, creation, _) = fixture();
    let created = state
...
fn sign_remove(request: &mut remove_liquidity::Request) {
    ...
    request.native.witnesses.owners[0] = signature(&auth, &identities()[0].1);
}
...
#[test]
fn owner_redeems_partial_then_all_owned_lp_leaving_minimum_locked() {
    let (mut state, pool, reserve, funding) = initialized();
    let initial_lp = state.blch_lp_position(&pool, &identities()[0].0);
    let supply = state
        .native()
        .gateway()
        .native()
        .supply(&state.paired_custody(&reserve).unwrap().asset);
    let first = remove_request(&state, pool, reserve, funding, initial_lp / 2);
    let expected = state.quote_blch_remove(&first.quote, 4).unwrap();
    let total_lp = state.blch_pool(&pool).unwrap().lp_supply();
    for i in 0..2 {
        assert_eq!(
            expected.amounts_out[i],
            (u128::from(first.quote.lp) * u128::from(expected.reserves_before[i])
                / u128::from(total_lp)) as u64
        );
    }
    let first_receipt = state
        .execute_blch_remove(&first, 4, &BaseVerifier, &BlochVerifier)
        .unwrap();
    assert_eq!(first_receipt.quote, expected);
    assert_eq!(
        state.blch_lp_position(&pool, &identities()[0].0),
        initial_lp - first.quote.lp
    );
    assert_eq!(
        state
            .base()
            .utxo(&first_receipt.blch_txid, 1)
            .unwrap()
            .value,
        expected.amounts_out[0]
    );
    assert_eq!(
        state
            .native()
            .gateway()
            .native()
            .output(&first_receipt.native.outputs[1])
            .unwrap()
            .output
            .amount,
        expected.amounts_out[1]
    );
    reject_remove(&mut state, &first);
    ...
    assert_eq!(
        restored.blch_lp_position(&pool, &identities()[0].0),
        initial_lp - first.quote.lp
    );
    state = restored;
    let remaining = state.blch_lp_position(&pool, &identities()[0].0);
    let second = remove_request(
        &state,
        pool,
        reserve,
        (first_receipt.blch_txid, 2),
        remaining,
    );
    let second_receipt = state
        .execute_blch_remove(&second, 4, &BaseVerifier, &BlochVerifier)
        .unwrap();
    assert_eq!(second_receipt.quote.lp_remaining, 0);
    assert_eq!(state.blch_lp_position(&pool, &identities()[0].0), 0);
    assert_eq!(state.blch_lp_position(&pool, &identities()[2].0), 0);
    assert_eq!(state.blch_pool(&pool).unwrap().lp_supply(), 1_000);
    assert_eq!(
        state.blch_pool(&pool).unwrap().reserves(),
        second_receipt.quote.reserves_after
    );
    assert!(second_receipt.quote.reserves_after.iter().all(|v| *v > 0));
    ...
    assert_eq!(base_value + fees + tips, 2 * u128::from(COIN));
    let restored = State::restore(state.snapshot(), state.state_root(), &BlochVerifier).unwrap();
    assert_eq!(restored.state_root(), state.state_root());
    assert_eq!(restored.blch_lp_position(&pool, &identities()[0].0), 0);
    reject_remove(&mut state, &second);
    let mut excess = second.quote.clone();
    excess.revision = state.blch_pool(&pool).unwrap().revision();
    excess.lp = 1;
    assert!(state.quote_blch_remove(&excess, 4).is_err());
    // Locked minimum liquidity keeps the pool operational after the owner exits.
    let swap = swap_request(&state, pool, reserve, ([9; 32], 0), None);
    let swapped = state
        .execute_blch_swap(&swap, 4, &BaseVerifier, &BlochVerifier)
        .unwrap();
    assert_eq!(
        state.blch_pool(&pool).unwrap().reserves(),
        swapped.quote.reserves_after
    );
    assert_eq!(state.blch_pool(&pool).unwrap().lp_supply(), 1_000);
    assert_eq!(state.blch_lp_position(&pool, &identities()[0].0), 0);
    let restored = State::restore(state.snapshot(), state.state_root(), &BlochVerifier).unwrap();
...
#[test]
fn forged_signatures_wrong_owner_slippage_and_redirects_reject_atomically() {
    let (mut state, pool, reserve, funding) = initialized();
    let valid = remove_request(&state, pool, reserve, funding, 100_000);
    for offset in [
    ...
    } = &mut bad.blch
    {
        ...
        inputs[1].txid = [9; 32];
        inputs[1].vout = 0;
        outputs[1].script_hash = Sha3_256::digest(&identities()[2].0).into();
        outputs[2].script_hash = outputs[1].script_hash;
    }
    bad.native.transaction.outputs[1].owner = identities()[2].0.clone();
    let fee = state.quote_blch_remove_fee(&bad).unwrap();
    if let PosTransaction::TransferV2 { outputs, .. } = &mut bad.blch {
        outputs[2].value = COIN - (fee.base_fee_sat + fee.priority_fee_sat) as u64;
    }
    let hash = bad.authorization(&DOMAIN).unwrap();
    if let PosTransaction::TransferV2 { keys, .. } = &mut bad.blch {
        keys[0].signature = signature(&hash, &identities()[2].1);
    }
    bad.native.witnesses.owners[0] = signature(&hash, &identities()[2].1);
    reject_remove(&mut state, &bad);
    for leg in 0..2 {
        let mut bad = valid.clone();
        bad.quote.minimum[leg] = u64::MAX;
        sign_remove(&mut bad);
        reject_remove(&mut state, &bad);
    }
    let mut bad = valid.clone();
    ...
    sign_remove(&mut bad);
    reject_remove(&mut state, &bad);
    // Even the legitimate owner cannot pay fees by silently reducing the quoted payout.
    let mut bad = valid.clone();
    if let PosTransaction::TransferV2 { outputs, .. } = &mut bad.blch {
        outputs[1].value -= 1;
        outputs[2].value += 1;
    ...
    reject_remove(&mut state, &bad);
    state
        .execute_blch_remove(&valid, 4, &BaseVerifier, &BlochVerifier)
        .unwrap();
}
