...
| BV-09 | `VaultKeys` clears its owned PQ secret vector on drop and no longer implements whole-aggregate `Clone`. Temporary V2/V3 PQ seeds and the BTC identity helper's discarded PQ secret are zeroizing. | Public fields still permit explicit copies; classical `SecretKey` values are Copy and use the library's explicitly best-effort erase. Caller copies, BIP32 intermediate objects, registers, allocator/OS copies and third-party crypto internals are outside this guarantee. |
...
- `VaultKeys` no longer implements `Clone`. The repository has no remaining
  consumer, but this is a source-breaking change for unknown external Rust
  consumers of the internal crate; such callers must redesign ownership rather
  than recreate a convenience copy of every secret.
