...
//! SEPARATE method (`Block::validate_pow`) that reads only header/bits/
//! pow_solution and NEVER the `transactions`. Therefore a transaction that is
//! valid today (pure SHA-256d, empty `pow_solution`) is valid BYTE-FOR-BYTE
//! inside a Dual-AND block (SHA-256d + a bound SIS witness). Adding the SIS
//! co-requirement creates NO transaction-compatibility problem.
//!
//! This test does NOT modify consensus. It reconstructs the Dual-AND predicate
//! from EXISTING public primitives (same style as `tests/dual_and_local.rs` and
//! `examples/mine_dual_and.rs`) and drives the REAL transaction-level validators
//! on ONE block whose ONLY difference between the two configs is `pow_solution`.
//!
//! LOCATION NOTE: this test lives in the `bloch-crypto` crate (not the root
//! `bloch` crate) because on this integration branch the root `bloch` lib does
//! not compile — an unrelated, uncommitted WIP in `src/network` references a
//! nonexistent `NetworkConfig.archive` field and uses `SyncEntry` in
//! `#[derive(PartialEq, Eq)]` without those impls. That breakage is NOT
//! consensus code and NOT part of this task; rather than complete someone's
//! half-finished feature, the test targets `bloch-crypto`, which owns every
//! primitive under test (Block/Transaction/wallet/crypto + the `bloch-sis-pow`
//! dependency) and builds cleanly on its own. The Dual-AND primitives used here
//! are the exact ones `src/pow::{mine,verify}_sis_pow_testnet` wrap
//! (`bloch_sis_pow::{solver::mine, verify_regime}` with k = TESTNET_RESIDUAL_
//! COEFFS = 4, candidates_per_nonce = 4096).
//!
//! CORRECTION vs the task grounding: `Block::validate_structure` is NOT a pure
//! tx-level check — it CALLS `validate_pow()` internally. It is the single block
//! method where PoW enters, so it is deliberately EXCLUDED from the invariant
//! set and the PoW predicate is reconstructed separately. That exclusion IS the
//! point: PoW is cleanly factored out of transaction validation.
...
use bloch_crypto::wallet::{generate_keypair, TxBuilder};

use bloch_sis_pow::params::N as SOLUTION_LEN; // 256
use bloch_sis_pow::solver::{mine, MineConfig};
use bloch_sis_pow::{bits_to_target, target_to_bits, verify_regime, Target, TESTNET_RESIDUAL_COEFFS};
...
// ── Local replicas of src/pow::{verify,mine}_sis_pow_testnet ────────────────
// (the root `pow` module is behind the `node` feature, which drags in the
//  broken network code; these wrap the same `bloch-sis-pow` calls identically.)

fn verify_sis_pow_testnet(preimage: &[u8], nonce: u64, s: &[i32; SOLUTION_LEN], bits: u32) -> bool {
    verify_regime(preimage, nonce, s, &bits_to_target(bits), TESTNET_RESIDUAL_COEFFS).is_ok()
}

/// One-nonce-bound SIS solve (budget == candidates_per_nonce == 4096, exactly
/// as `mine_sis_pow_testnet(.., start_nonce, 4096)` — so a found witness binds
/// THIS nonce and cannot wander to another).
fn mine_sis_bound_to_nonce(preimage: &[u8], bits: u32, nonce: u64) -> Option<[i32; SOLUTION_LEN]> {
    let cfg = MineConfig {
        start_nonce: nonce,
    ...
    match mine(preimage, &bits_to_target(bits), &cfg, None) {
        Ok(r) => {
            assert_eq!(r.nonce, nonce, "coupling: SIS witness must bind the SHA nonce");
            Some(r.solution)
        }
        Err(_) => None,
    }
...
fn dual_and_pow_ok(header: &BlockHeader, pow_solution: &[i32], sha_target: &[u8; 32], sis_bits: u32) -> bool {
    ...
    if pow_solution.len() != SOLUTION_LEN {
        return false;
    }
    let mut s = [0i32; SOLUTION_LEN];
    s.copy_from_slice(pow_solution);
    verify_sis_pow_testnet(&header.pow_preimage(), header.nonce, &s, sis_bits)
}
...
fn tx_validity_is_invariant_to_dual_and_pow() {
    ...
    let sis_bits = target_to_bits(&Target::MAX);
    ...
    let (shared_nonce, witness) = {
        ...
        loop {
            ...
            if sha256d_pow_valid(&h.pow_hash(), &sha_target, HEIGHT) {
                // SIS solve BOUND to THIS exact nonce.
                if let Some(sol) = mine_sis_bound_to_nonce(&preimage, sis_bits, nonce) {
    ...
    assert_eq!(
        ...
        "the tx-level checks must all PASS: (merkle, coinbase_format, no_dup, dust, signature)"
    );
    ...
    let mut s_arr = [0i32; SOLUTION_LEN];
    s_arr.copy_from_slice(&block_b.pow_solution);
    assert!(
        verify_sis_pow_testnet(&block_b.header.pow_preimage(), block_b.header.nonce, &s_arr, sis_bits),
