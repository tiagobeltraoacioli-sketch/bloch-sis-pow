...
mod tests {
    ...
    /// epoch (the partition spreads the 4 fixture validators over the first
    /// slots of the epoch, one member each), signed by the mock suite — the
    /// same committee rule `validate_included_attestation` and the
    /// transition's step 8 enforce since the 2026-08-22 alignment.
    ...
    ///
    /// The divergence was recorded in derive.rs on 2026-08-12 and was
    /// harmless only while gossip pools stayed empty. The shared-basis
    /// gossip fix fills them, and a producer filtering by the wrong draw
    /// then assembles blocks the whole network refuses at step 8
    /// (`TransitionError::Attestation`) — the h28080 shape, one layer up.
    ///
    /// Both halves are here: the sampled-only validator must be DROPPED, and
    /// the real partition member must be KEPT (a filter that drops
    ...
    fn the_inclusion_filter_draws_the_partition_committee_not_the_sampled_one() {
        ...
        assert!(
            derive::validate_included_attestation(&parent, SLOT, &vote(member), &MockCrypto).is_ok(),
