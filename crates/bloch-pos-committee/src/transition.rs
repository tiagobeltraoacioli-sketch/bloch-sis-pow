...
impl PosTransaction {
    ...
    pub fn spend_signing_root(&self) -> [u8; 32] {
        ...
        match self {
            ...
            PosTransaction::FundedDeposit {
                ...
                // domain then rests on: (1) this stream begins 0x06 while a
                // Transfer stream begins with a 4-byte input count, (2) the
    ...
    pub fn from_canonical_bytes(bytes: &[u8]) -> Result<Self, TxDecodeError> {
        ...
        let tx = match tag {
            ...
            // The funded staking shapes (flag day:
            // `params::FUNDED_STAKE_ACTIVATION_EPOCH`). Note what is NOT
            // here: no gate check. Decodability is permanent for every tag
            // this build has ever known — historical blocks must re-decode on
            // every replay — and validity is a transition rule
            // (`apply_transaction`'s epoch dispatch), never a decode rule.
            // The legacy tags 0x02–0x04 above stay decodable forever for the
            // same reason, even after the gate makes them invalid in new
            // block bodies.
            0x06 => {
            ...
            0x07 => PosTransaction::SignedExit {
                validator: r.u32()?,
                epoch: r.u64()?,
                signature: r.bytes()?,
            },
...
mod tx_codec_tests {
    ...
    fn samples() -> Vec<PosTransaction> {
        vec![
            ...
            // The funded staking shapes (0x06–0x08). Same value discipline as
    ...
    fn funded_shapes_refuse_truncation_and_trailing_bytes() {
        ...
        // Control for the slice above: samples()[6..] really are the funded
        // shapes, or the loop asserted over nothing.
        assert!(matches!(samples()[6], PosTransaction::FundedDeposit { .. }));
        assert!(matches!(samples()[9], PosTransaction::Withdraw { .. }));
    }
