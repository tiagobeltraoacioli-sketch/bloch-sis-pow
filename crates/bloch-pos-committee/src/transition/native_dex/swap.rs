//! Atomic exact-input BLCH/native swaps in the default-off combined rehearsal.
...
pub const SWAP_GAS: u64 = 5_000;

#[derive(Clone, Debug)]
pub struct Request {
    pub quote: swap_quote::Request,
...
impl Request {
    ...
    fn intent_bytes(&self) -> Vec<u8> {
        ...
        bytes.extend_from_slice(&self.quote.input_asset);
        bytes.extend_from_slice(&self.quote.amount.to_le_bytes());
        bytes.extend_from_slice(&self.quote.minimum_out.to_le_bytes());
        bytes
    }
    pub fn canonical_bytes(&self, domain: &[u8; 32]) -> Result<Vec<u8>, Error> {
        ...
        bytes[..8].copy_from_slice(b"BLCHSWAP");
    ...
    pub fn authorization(&self, domain: &[u8; 32]) -> Result<[u8; 32], Error> {
        ...
        h.update(b"BLOCH-BLCH-SWAP-AUTH-v1");
    ...
    pub fn output_txid(&self, domain: &[u8; 32]) -> Result<[u8; 32], Error> {
        let mut h = Sha3_256::new();
        h.update(b"BLOCH-BLCH-SWAP-OUT-v1");
...
impl State {
    pub fn quote_blch_swap_fee(&self, request: &Request) -> Result<fee_market::TxCharge, Error> {
        self.quote_blch_swap_fee_with_context(request, self.base.next_base_fee(), 0)
    }
    pub(in crate::transition::native_dex) fn quote_blch_swap_fee_with_context(
        &self,
        request: &Request,
        base_fee: u128,
        outer_bytes: u64,
        ...
        let gas = fee_market::intrinsic_gas(fee_market::TxClass::Eutxo { inputs: 1 }, *tx_bytes)
            ...
            .and_then(|g| g.checked_add(SWAP_GAS))
    ...
    pub fn execute_blch_swap(
        ...
        self.execute_blch_swap_with_context(
            request,
            height,
    ...
    pub(in crate::transition::native_dex) fn execute_blch_swap_with_context(
        &mut self,
        request: &Request,
        height: u64,
        base_fee: u128,
        ...
        let charge = self.quote_blch_swap_fee_with_context(request, base_fee, outer_bytes)?;
        let quote = self.quote_blch_swap(&request.quote, height)?;
        ...
        let blch_in = request.quote.input_asset == bloch_euvm::BLCH;
        ...
        let change_index = if blch_in {
            1
        } else {
            if outputs.get(1).is_none_or(|o| o.value != quote.amount_out) {
                return Err(Error::InvalidReserve);
            }
            2
        };
        let change: u128 = outputs[change_index..]
            .iter()
            .map(|o| u128::from(o.value))
            .sum();
        let debit = if blch_in {
            u128::from(request.quote.amount)
        } else {
            0
        };
        // Swap proceeds and the reserve cannot pay the network fee.
        if base_funding != change + debit + charge.base_fee_sat + charge.priority_fee_sat {
        ...
        if blch_in {
            if tx.inputs.len() != 1
                || tx.outputs.len() != 2
                || tx.outputs[1].amount != quote.amount_out
            {
                return Err(Error::InvalidReserve);
            }
        } else {
            let change: u128 = tx.outputs[1..].iter().map(|o| u128::from(o.amount)).sum();
            if native_funding != change + u128::from(request.quote.amount) {
                return Err(Error::InvalidReserve);
            }
        }
        ...
        let permit = ReserveSpend::swap(self, &quote, authorization)?;
        let base_fees = self
            .base_fees
            ...
            .transition(
                &amm::Request {
                    ...
                    action: amm::Action::SwapExactInput {
                        input_index: if blch_in { 0 } else { 1 },
                        amount: request.quote.amount,
                        minimum_out: request.quote.minimum_out,
                    },
                },
                height,
            )
