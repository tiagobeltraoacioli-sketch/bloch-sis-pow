...
const TAG_EVM_COMMITMENT: u8 = 0x09;
...
mod tests {
    ...
    fn state(f: &Fx) -> ConsensusState<'_> {
        ConsensusState {
            ...
            evm: EvmCommitment {
                account_root: val(104),
                receipts_root: val(105),
                gas_used: 21_000,
                base_fee_per_gas: 1,
            },
        }
    ...
    fn every_component_field_is_load_bearing() {
        ...
        // Singleton roots and the EVM commitment. Every field of the EVM leaf
        // must be load-bearing for the same reason as every other field: an
        // uncommitted `base_fee_per_gas` would be a mutable retarget input,
        // the `expected_bits` failure shape.
        for i in 0..7 {
            let f2 = f.clone();
            let mut s = state(&f2);
            match i {
                ...
                3 => s.evm.account_root[0] ^= 1,
                4 => s.evm.receipts_root[0] ^= 1,
                5 => s.evm.gas_used += 1,
                _ => s.evm.base_fee_per_gas += 1,
            }
            let r = state_root(&s);
            assert_ne!(r, base);
            roots.push(r);
