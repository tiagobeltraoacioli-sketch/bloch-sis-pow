...
/// Flag day for the withdrawal transaction (tag `0x07`,
...
/// INERT — the old binary fails to decode tag `0x07`
...
///
/// 1. **Deposits must debit the eUTXO set first (or on the same flag day).**
///    Today `PosTransaction::Deposit` names an `amount_sat` and spends no
///    output — bonded stake is created without destroying spendable coins
///    (the gap `transition.rs` documents on the `eutxos` field, and the
///    reason the node refuses deposits at admission, `engine.rs::admissible`:
///    measured 2026-08-13 at 25,000 BLOCH of stake per unauthenticated
///    request). While that gap is open, deposit → exit → withdraw is a
///    money printer: the depositor keeps the coins it "deposited" AND is
///    paid the bond as fresh eUTXO value. Activating withdrawals without
///    funded deposits converts an accounting looseness into direct
///    inflation. The node-side refusal of deposits is NOT a sufficient
///    guard — it is policy, not consensus, and any proposer can include a
///    deposit regardless.
