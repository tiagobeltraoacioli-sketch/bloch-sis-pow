...
mod tests {
    ...
    fn v2_era_doubles_the_cap() {
        assert_eq!(max_block_tx_bytes(u64::MAX), 524_288);
        assert_eq!(block_tx_bytes_target(u64::MAX), 262_144);
    }
    ...
    fn the_cap_and_the_target_are_one_switch_not_two() {
        ...
        assert!(
            next_base_fee(8_000, saturated, 0) > 8_000,
        ...
        assert_eq!(
            next_base_fee(8_000, saturated, u64::MAX),
            8_000,
    ...
    fn a_saturated_v2_payload_still_fits_under_the_gas_cap() {
        ...
        let payload_gas = max_block_tx_bytes(u64::MAX) * GAS_PER_BYTE;
    ...
    fn raising_the_block_cap_is_a_flag_day() {
        assert_eq!(
            ...
            u64::MAX,
            "activating the 512 KiB cap is a flag day: set the epoch and roll \
             the fleet together, and rebuild BEFORE lowering it"
        );
    }
