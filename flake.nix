{
  ...
  #
  # RESOLVED REVS (recorded from the committed flake.lock generated by the human on
  # the aarch64 Nix host — do NOT hand-edit a flake.lock here; agents don't run Nix
  # and full narHashes must never be fabricated. Regenerate with scripts/flake-lock.sh):
  #     nixpkgs      rev: ac62194  (branch nixos-25.05 at lock time)
  #     mobile-nixos rev: 458448d
  # To make the mobile-nixos pin a single diffable line, the human replaces the URL
  # below with the full 40-char rev (available from `nix flake metadata`) once the
  # lock lands, then re-runs `nix flake lock`.
  #
