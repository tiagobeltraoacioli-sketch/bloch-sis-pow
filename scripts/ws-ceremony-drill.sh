...
# generated inside a newly created private workdir. An explicit workdir must
# not exist. Disposable secret keys are unlinked on exit (not secure erasure);
# public artifacts remain for inspection. It reads two archival RPCs.
...
if [ -n "${2:-}" ]; then
  WORK="$2"
  mkdir -m 700 -- "$WORK" || { echo "FATAL: workdir must be new: $WORK" >&2; exit 1; }
else
  WORK="$(mktemp -d "${TMPDIR:-/tmp}/ws-drill.XXXXXX")" || exit 1
fi
cleanup_disposable_keys() {
  local rc=$?
  trap - EXIT
  # Only these four files can be generated by this drill. Never glob or
  # recurse through an operator-supplied directory during cleanup.
  if ! rm -f -- "$WORK/s0.sk" "$WORK/s1.sk" "$WORK/s2.sk" "$WORK/outsider.sk"; then
    echo "FATAL: could not remove disposable signer keys in $WORK" >&2
    rc=1
  fi
  exit "$rc"
}
trap cleanup_disposable_keys EXIT
trap 'exit 129' HUP
trap 'exit 130' INT
trap 'exit 143' TERM
