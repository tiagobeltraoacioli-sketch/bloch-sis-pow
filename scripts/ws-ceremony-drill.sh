...
expect_ok "assemble the Phase A arrangement (2-of-3, >=1 external)" -- \
  "$BIN" ws-signer-set --id 1 --threshold 2 --min-external 1 --adopted-epoch 0 \
...
for i in 0 1 2; do
  "$BIN" ws-sign --key "$WORK/s$i.sk" --pubkey "$WORK/s$i.pk" \
    --checkpoint "$CP_BIN" --out "$WORK/sig$i" >/dev/null || exit 1
done
...
expect_refusal "assembling with a mispaired signer index" "BadSignature" -- \
  "$BIN" ws-envelope --checkpoint "$CP_BIN" --signer-set "$WORK/set1.bin" \
...
"$BIN" ws-sign --key "$WORK/outsider.sk" --pubkey "$WORK/outsider.pk" \
  --checkpoint "$CP_BIN" --out "$WORK/sig-out" >/dev/null
...
"$BIN" ws-signer-set --id 2 --threshold 2 --min-external 1 --adopted-epoch 0 \
  --signer "$WORK/s0.pk:internal" --signer "$WORK/s1.pk:internal" \
...
expect_refusal "minting signer-set id 0" "reserved" -- \
  "$BIN" ws-signer-set --id 0 --threshold 2 --min-external 1 --adopted-epoch 0 \
    --signer "$WORK/s0.pk:internal" --signer "$WORK/s1.pk:internal" \
...
for i in 0 2; do
  "$BIN" ws-sign --key "$WORK/s$i.sk" --pubkey "$WORK/s$i.pk" \
    --checkpoint "$WORK/cp-far.bin" --out "$WORK/sigfar$i" >/dev/null
  "$BIN" ws-sign --key "$WORK/s$i.sk" --pubkey "$WORK/s$i.pk" \
    --checkpoint "$WORK/cp-far1.bin" --out "$WORK/sigfar1$i" >/dev/null
done
...
# (n) a checkpoint older than the window. This is NOT an envelope rule — the
