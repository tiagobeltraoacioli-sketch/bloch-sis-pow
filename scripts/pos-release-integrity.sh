...
# only way to learn whether a box would follow a flag day was to arm it and
...
#
# The character class MUST include digits: `[A-Z_]*_ACTIVATION_EPOCH` misses
# BLOCK_BYTES_V2_ACTIVATION_EPOCH, which is armed at epoch 800 — a live gate,
# so the undercount would be silent and wrong in the dangerous direction.
DECLARED="$(grep -c '^pub const [A-Za-z0-9_]*_ACTIVATION_EPOCH' \
  "$COMMITTEE_DIR/src/params.rs" || true)"
# `grep -o | wc -l` counts OCCURRENCES; `grep -c` counts matching LINES and
# would silently return 1 if the serializer ever put the gates on one line.
STATED="$(printf '%s' "$GJ" | grep -o '"name":' | wc -l | tr -d ' ')"
[ "$DECLARED" -gt 0 ] \
