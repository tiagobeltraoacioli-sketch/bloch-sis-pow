...
probe() {
  ...
  #
  # WHAT WE ASK, AND WHY IT IS NOT THE PATH IN THE TABLE
  # RELEASE-INTEGRITY.md §4: "the authoritative answer to what is this node
  # running is the kernel, not the unit file". The same applies here, one
  # level down — a path in a hand-maintained TSV is an even weaker claim than
  # a unit file. If the on-disk file was replaced after the node started, the
  # process is still running the OLD bytes, and asking the file would produce
  # a confident answer about a binary nobody is running. That is the precise
  # shape of the failure this tool exists to prevent.
  #
  # So: resolve the RUNNING image from /proc and ask THAT. `/proc/PID/exe` is
  # executable even when the file it points at was replaced or deleted. The
  # table path is the fallback, used only when no node is running (a release
  # artifact staged but not yet started), and the report always says which
  # source answered. If one box is running two different images, we refuse to
  # pick one — that is the "split the row" case the host table describes.
  local remote='
    set -u
    CONF=__BIN__
    EXES=$(for p in $(pgrep -x -f "[^ ]*bloch-pos[^ ]* run .*" 2>/dev/null \
                      || pgrep -f "bloch-pos.* run " 2>/dev/null); do
             readlink "/proc/$p/exe" 2>/dev/null && echo " $p"
           done | paste - - 2>/dev/null | sort -u -k1,1)
    N=$(printf "%s" "$EXES" | grep -c . || true)
    if [ "$N" -gt 1 ]; then
      echo "SWEEP-ERROR: this box runs $N DIFFERENT node images; one row cannot"
      echo "describe it. Split the host-table row per binary. Images seen:"
      printf "%s\n" "$EXES"
      exit 0
    fi
    if [ "$N" -eq 1 ]; then
      PID=$(printf "%s" "$EXES" | awk "{print \$2}")
      SRC="/proc/$PID/exe"
      echo "GATE-SWEEP-SOURCE: running image, pid $PID, exe $(readlink /proc/$PID/exe 2>/dev/null)"
    else
      SRC="$CONF"
      echo "GATE-SWEEP-SOURCE: no running node found; asked the configured path $CONF"
    fi
    "$SRC" selfcheck --json 2>&1
  '
  remote="${remote//__BIN__/\'$bin\'}"
  $TIMEOUT_CMD ssh -n \
      -i "$keypath" \
      -o BatchMode=yes \
