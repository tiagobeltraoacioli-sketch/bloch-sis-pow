...
package = site/'downloads/bloch-data-local-workbench-v1.zip'
files = [site/'index.html', site/'README.md', site/'GOVERNANCE.md', site/'regulatory-register.v1.json']
files += [site/'assets'/name for name in ['exchange.v1.css','modules.v1.mjs','reconcile.v1.mjs','samples.v1.mjs','workbench.v1.mjs','bloch-inc-logo-dark.png','favicon.png']]
files += sorted((site/'samples').glob('*.csv'))
with zipfile.ZipFile(package, 'w', compression=zipfile.ZIP_DEFLATED) as archive:
    for file in files:
        data = file.read_bytes()
        if file.name == 'index.html':
            data = data.replace(b'href="downloads/bloch-data-local-workbench-v1.zip" download', b'href="README.md"')
