...
def snapshot(rpc, withdrawal, spend, destination, amount):
    ...
    for field in ('block_id', 'state_root', 'slot', 'finalized'):
        require(field in before and before[field] == after.get(field), 'Node advanced during observation; retry.')
...
def verify(first, second, withdrawal, spend, destination, amount):
    ...
    require(observations[0] == observations[1], 'Nodes disagree or were observed at different heads; retry.')
...
def main():
    ...
    result = verify(RPC(a.rpc_a), RPC(a.rpc_b), a.withdrawal_txid, a.spend_txid, a.destination, a.value_sat)
