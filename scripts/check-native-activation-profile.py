...
def validate(path, now=None):
    ...
    require(len(set(epochs.values())) == 1, "default policy requires all six gates at the same epoch; staged plans require separate review")
    require(epochs["withdrawal"] <= epochs["pool"], "pool cannot precede native withdrawal")
    ...
    finalized_slot = integer(observation.get("finalizedSlot"), "finalizedSlot")
    require(observation.get("finalityRule") == "canonical-checkpoint-slot-v1", "checkpoint-slot finality observation required")
    require(finalized_slot <= finalized * 32, "checkpoint slot exceeds its finalized epoch boundary")
    require(0 <= head - finalized_slot <= 96, "finalized checkpoint is ahead of head or more than three epochs behind")
    ...
    for reference in references:
        ...
        require(isinstance(checks, dict) and set(checks) == {"canonicalReplayPassed", "historicalRootsUnchanged", "rollbackPrepared", "withdrawalSimulationPassed", "sourceReleaseRehearsalPassed"} and all(value is True for value in checks.values()), "required readiness checks missing")
